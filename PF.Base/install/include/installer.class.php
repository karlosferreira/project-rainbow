...
class Phpfox_Installer
{
    ...
    function _load_general_steps()
    {
        ...
        if ($this->_bUpgrade && setting('pf_assets_storage_id')) {
            $aSteps[] = [
    ...
    {
        if ($this->_bUpgrade &&  !empty($assetStorageId = setting('pf_assets_storage_id'))) {
            if (!empty(request()->get('progress'))) {
                $files = request()->get('files');
                $storageId = request()->get('storage_id');
                $response = [];
                if (!empty($files) && !empty($storageId) && $storageId == $assetStorageId && !empty($storage =  Phpfox::getLib('storage')->get($storageId))) {
                    $response =  [];
