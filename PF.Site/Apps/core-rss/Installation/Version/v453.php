...
class v453
{
    ...
    private function _importForumToRssFeed()
    {
        ...
        if (!$iCntRss) {
            ...
            Lib::phrase()->addPhrase('forum_rss_title_' . $iGroupId, 'Latest Forum Topics');
            Lib::phrase()->addPhrase('forum_rss_description_' . $iGroupId,
                'List of the latest topics from our public forum.');
            db()->update(':rss_group', ['name_var' => 'forum.rss_group_name_' . $iGroupId],
                'group_id =' . $iGroupId);
            $aRssData['title_var'] = 'forum_rss_title_' . $iGroupId;
            $aRssData['description_var'] = 'forum_rss_description_' . $iGroupId;
    ...
    private function _importEventToRssFeed()
    {
        ...
        if (!$iCntRss) {
            ...
            Lib::phrase()->addPhrase('event_rss_title_' . $iGroupId, 'Latest Events');
            Lib::phrase()->addPhrase('event_rss_description_' . $iGroupId, 'List of all the upcoming events.');
            db()->update(':rss_group', ['name_var' => 'event.rss_group_name_' . $iGroupId], 'group_id =' . $iGroupId);
            $aRssData['title_var'] = 'event_rss_title_' . $iGroupId;
            $aRssData['description_var'] = 'event_rss_description_' . $iGroupId;
            $aRssData['group_id'] = $iGroupId;
            db()->insert(':rss', $aRssData);
        } else {
