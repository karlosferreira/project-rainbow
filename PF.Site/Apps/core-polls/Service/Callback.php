...
class Callback extends \Phpfox_Service
{
    ...
    public function getActivityFeedComment($aRow)
    {
        ...
        $aItem = $this->database()->select('b.poll_id, b.module_id, b.item_id, b.privacy, b.question, b.time_stamp, b.total_comment, b.total_like, c.total_like, ct.text_parsed AS text, f.friend_id AS is_friend, ' . Phpfox::getUserField())
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify([
                ...
                'owner_subject' => ['full_name_commented_on_one_of_your_polls_title',
                    ['full_name' => Phpfox::getUserBy('full_name'), 'title' => $aPoll['question']]],
                'owner_message' => ['full_name_commented_on_your_poll_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                    [
                ...
                'mass_id' => 'poll',
                'mass_subject' => (Phpfox::getUserId() == $aPoll['user_id'] ? ['full_name_commented_on_gender_poll',
                    [
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => \Phpfox::getService('user')->gender($aPoll['gender'], 1)
                    ]]
                    : ['full_name_commented_on_other_full_name_s_poll', [
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'other_full_name' => $aPoll['full_name']
                    ]]),
                'mass_message' => (Phpfox::getUserId() == $aPoll['user_id'] ?
                    ['full_name_commented_on_gender_poll_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        [
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'gender' => \Phpfox::getService('user')->gender($aPoll['gender'], 1),
                            'link' => $sLink,
                    ...
                    :
                    ['full_name_commented_on_other_full_name_s_poll_a_href_link_title_a_to_see_the_comment_thread_follow_the_link_below_a_href_link_link_a',
                        [
                            'full_name' => Phpfox::getUserBy('full_name'),
                            'other_full_name' => $aPoll['full_name'],
                            'link' => $sLink,
    ...
    public function getCommentNotification($aNotification)
    {
        ...
        if ($aNotification['user_id'] == $aRow['user_id'] && !isset($aNotification['extra_users'])) {
            $sPhrase = _p('user_name_commented_on_gender_poll_title', [
            ...
            $sPhrase = _p('user_name_commented_on_your_poll_title', [
                'user_name' => \Phpfox::getService('notification')->getUsers($aNotification),
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['question'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
            ]);
        } else {
            $sPhrase = _p('user_name_commented_on_span_class_drop_data_user_full_name_s_span_poll_title', [
                'user_name' => \Phpfox::getService('notification')->getUsers($aNotification),
    ...
    public function getItemName($iId, $sName)
    {
        return '<a href="' . Phpfox_Url::instance()->makeUrl('comment.view',
                ['id' => $iId]) . '">' . _p('on_name_s_poll', ['name' => $sName]) . '</a>';
    }
    ...
    public function deleteComment($iPoll)
    {
        ...
        $this->database()->update($this->_sTable, ['total_comment' => ['= total_comment -', 1]],
            'poll_id = ' . (int)$iPoll);
    ...
    public function getNewsFeed($aRow)
    {
        ...
        $aRow['text'] = _p('a_href_user_link_full_name_a_added_a_new_poll_a_href_question_url_question_a', [
    ...
    public function pendingApproval()
    {
        return [
            ...
            'link' => Phpfox_Url::instance()->makeUrl('poll', ['view' => 'pending'])
        ];
    }
    ...
    public function getNewsFeedFeedLike($aRow)
    {
        if ($aRow['owner_user_id'] == $aRow['viewer_user_id']) {
            $aRow['text'] = _p('a_href_user_link_full_name_a_likes_their_own_a_href_link_poll_a', [
    ...
    {
        return [
            'message' => _p('a_href_user_link_full_name_a_likes_your_a_href_link_poll_a', [
                    'full_name' => Phpfox::getLib('parse.output')->clean($aRow['full_name']),
                    'user_link' => Phpfox_Url::instance()->makeUrl($aRow['user_name']),
                    'link' => Phpfox_Url::instance()->makeUrl('poll', ['redirect' => $aRow['item_id']])
                ]
            ),
            'link' => Phpfox_Url::instance()->makeUrl('poll', ['redirect' => $aRow['item_id']])
        ];
    ...
    {
        return _p('a_href_user_link_full_name_a_likes_your_a_href_link_poll_a', [
                'full_name' => Phpfox::getLib('parse.output')->clean(Phpfox::getUserBy('full_name')),
                'user_link' => Phpfox_Url::instance()->makeUrl(Phpfox::getUserBy('user_name')),
                'link' => Phpfox_Url::instance()->makeUrl('poll', ['redirect' => $iItemId])
            ]
        );
    }
    ...
    public function getActivityFeed($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        if ($aRow['module_id'] && Phpfox::hasCallback($aRow['module_id'], 'checkPermission') && !Phpfox::callback($aRow['module_id'] . '.checkPermission', $aRow['item_id'], 'poll.view_browse_polls')) {
            return false;
        }
    ...
    public function getNotification($aNotification)
    {
        ...
        if ($aNotification['user_id'] == $aRow['user_id']) {
            $sPhrase = _p('user_name_voted_on_gender_poll_title', [
                'user_name' => \Phpfox::getService('notification')->getUsers($aNotification),
                'gender' => \Phpfox::getService('user')->gender($aRow['gender'], 1),
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['question'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
            ...
            $sPhrase = _p('user_name_voted_on_your_poll_title', [
                'user_name' => \Phpfox::getService('notification')->getUsers($aNotification),
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['question'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
            ]);
        } else {
            $sPhrase = _p('user_name_voted_on_span_class_drop_data_user_full_name_s_span_poll_title', [
                'user_name' => \Phpfox::getService('notification')->getUsers($aNotification),
                'full_name' => $aRow['full_name'],
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['question'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
    ...
    public function getNotificationLike($aNotification)
    {
        ...
        if ($aNotification['user_id'] == $aRow['user_id']) {
            $sPhrase = _p('user_name_liked_gender_own_poll_title', [
                'user_name' => \Phpfox::getService('notification')->getUsers($aNotification),
                'gender' => \Phpfox::getService('user')->gender($aRow['gender'], 1),
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['question'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
            ...
            $sPhrase = _p('user_name_liked_your_poll_title', [
                'user_name' => \Phpfox::getService('notification')->getUsers($aNotification),
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['question'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
            ]);
        } else {
            $sPhrase = _p('user_name_liked_span_class_drop_data_user_full_name_span_poll_title', [
                'user_name' => \Phpfox::getService('notification')->getUsers($aNotification),
                'full_name' => $aRow['full_name'],
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['question'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
        ...
        if (!$bDoNotSendEmail) {
            ...
            Phpfox::getLib('mail')->to($aRow['user_id'])
                ->subject([
                    ...
                    ['full_name' => Phpfox::getUserBy('full_name'), 'question' => $aRow['question']]
                ])
                ->message([
    ...
    public function getNotificationApproved($aNotification)
    {
        ...
        $sPhrase = _p('your_poll_title_has_been_approved', [
            'title' => Phpfox::getLib('parse.output')->shorten($aRow['question'],
                Phpfox::getParam('notification.total_notification_title_length'), '...')
        ]);
    ...
    public function getCommentNotificationTag($aNotification)
    {
        ...
        $sPhrase = _p('user_name_tagged_you_in_a_comment_in_a_poll', ['user_name' => $aRow['full_name']]);
    ...
    public function getToSponsorInfo($iId)
    {
        ...
        if (empty($aPoll)) {
            return ['error' => _p('sponsor_error_poll_not_found')];
        }

        $aPoll['title'] = _p('poll_sponsor_title', ['sPollTitle' => $aPoll['question']]);
        $aPoll['paypal_msg'] = _p('poll_sponsor_paypal_message', ['sPollTitle' => $aPoll['question']]);
    ...
    {
        return [
            'poll.share_polls' => _p('who_can_share_polls'),
            'poll.view_browse_polls' => _p('who_can_view_polls')
        ];
    }
    ...
    {
        if (!Phpfox::getService('pages')->hasPerm($aPage['page_id'], 'poll.view_browse_polls') || !Phpfox::getUserParam('poll.can_access_polls')) {
            return null;
        }

        $aMenus[] = [
            'phrase' => _p('polls'),
            'url' => Phpfox::getService('pages')->getUrl($aPage['page_id'], $aPage['title'], $aPage['vanity_url']) . 'poll/',
            'icon' => 'module/core-poll.png',
            'landing' => 'poll'
        ];
    ...
    {
        if (!Phpfox::getService('pages')->hasPerm($aPage['page_id'], 'poll.share_polls')
            || !Phpfox::getUserParam('poll.can_create_poll') || !Phpfox::getUserParam('poll.can_access_polls') || !Phpfox::getService('poll')->checkLimitation()) {
            return null;
        }

        return [
            [
                'phrase' => _p('menu_add_new_poll'),
                'url' => Phpfox::getLib('url')->makeUrl('poll.add', [
                    'module' => 'pages',
                    'item' => $aPage['page_id']
                ])
            ]
    ...
    {
        $aPolls = db()->select('poll_id')->from($this->_sTable)->where([
            'module_id' => $sType,
            'item_id' => $iId
        ])->executeRows();
        foreach ($aPolls as $aPoll) {
    ...
    {
        if (!Phpfox::getService('pages')->hasPerm($iPage, 'poll.view_browse_polls') || !Phpfox::getUserParam('poll.can_access_polls')) {
            return false;
        }
    ...
    {
        return ['poll.share_polls' => _p('who_can_share_polls')];
    }
    ...
    {
        if (!Phpfox::getService('groups')->hasPerm($aPage['page_id'], 'poll.view_browse_polls') || !Phpfox::getUserParam('poll.can_access_polls')) {
            return null;
        }

        $aMenus[] = [
            'phrase' => _p('polls'),
            'url' => Phpfox::getService('groups')->getUrl($aPage['page_id'], $aPage['title'], $aPage['vanity_url']) . 'poll/',
            'icon' => 'module/core-poll.png',
            'landing' => 'poll'
        ];
    ...
    public function getGroupSubMenu($aPage)
    {
        if (!Phpfox::getService('groups')->hasPerm($aPage['page_id'],
                'poll.share_polls') || !Phpfox::getUserParam('poll.can_create_poll') || !Phpfox::getService('poll')->checkLimitation()) {
            return null;
        }
    ...
    public function getNotificationNewItem_Groups($aNotification)
    {
        ...
        $aPoll = Phpfox::getService('poll')->getPollById($aNotification['item_id']);
        if (empty($aPoll) || empty($aPoll['item_id']) || ($aPoll['module_id'] != 'groups')) {
            return false;
        }

        $aRow = Phpfox::getService('groups')->getPage($aPoll['item_id']);

        if (!isset($aRow['page_id'])) {
            return false;
        }

        $sPhrase = _p('users_add_a_new_poll_in_the_group_title', [
        ...
        return [
            'link' => Phpfox::getLib('url')->permalink('poll', $aPoll['poll_id'], $aPoll['question']),
            'message' => $sPhrase,
            'icon' => Phpfox::getLib('template')->getStyle('image', 'activity.png', 'poll')
        ];
    }
