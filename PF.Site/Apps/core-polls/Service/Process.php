...
class Process extends \Phpfox_Service
{
    ...
    public function add($iUser, $aVals, $bIsUpdate = false)
    {
        ...
        $bIsCustom = !empty($aVals['is_custom']);
        if ($bIsCustom) {
            $aVals['randomize'] = '0';
        }
        ...
        if ($bIsUpdate) {
            ...
            (empty($aVals['module_id']) && Phpfox::isModule('feed') ? \Phpfox::getService('feed.process')->update('poll', $iId, $aVals['privacy'],
            ...
            if (!Phpfox::getUserParam('poll.poll_requires_admin_moderation') && !$bIsCustom) {
                if(empty($aVals['module_id']) && Phpfox::isModule('feed') && Phpfox::getParam('poll.poll_allow_create_feed_when_add_new_item', 1)) {
                    Phpfox::getService('feed.process')->add('poll', $iId, $aVals['privacy'],
                        (isset($aVals['privacy_comment']) ? (int)$aVals['privacy_comment'] : 0));
                } elseif (!empty($aVals['item_id'])) {
                    if (Phpfox::isModule('feed') && Phpfox::getParam('poll.poll_allow_create_feed_when_add_new_item', 1) && Phpfox::hasCallback($aVals['module_id'],'getFeedDetails')) {
                        Phpfox::getService('feed.process')
                            ->callback(Phpfox::callback($aVals['module_id'] . '.getFeedDetails', $aVals['item_id']))
                            ->add('poll', $iId, $aVals['privacy'],
                    ...
                    if (Phpfox::isModule('notification') && $aVals['module_id'] != '' &&
                        ...
                        Phpfox::callback($aVals['module_id'] . '.addItemNotification', [
                            ...
                            'item_perm' => 'poll.view_browse_polls',
                            'item_type' => 'poll',
                            'item_id' => $iId,
    ...
    public function moderatePoll($iPoll, $iResult)
    {
        ...
        if ($iResult == '0') {
            ...
            if (Phpfox::isModule('feed') && Phpfox::getParam('poll.poll_allow_create_feed_when_add_new_item', 1)) {
                if (!empty($aPoll['module_id']) && !empty($aPoll['item_id']) && Phpfox::hasCallback($aPoll['module_id'], 'getFeedDetails')) {
                    Phpfox::getService('feed.process')->callback(Phpfox::callback($aPoll['module_id'] . '.getFeedDetails',
                        $aPoll['item_id']))->add('poll', $iPoll, $aPoll['privacy'], $aPoll['privacy_comment'],
                        $aPoll['item_id'], $aPoll['user_id']);
                } elseif (empty($aPoll['module_id'])) {
                    Phpfox::getService('feed.process')->add('poll', $aPoll['poll_id'],
                        $aPoll['privacy'], $aPoll['privacy_comment'], 0, $aPoll['user_id']);
                }
            }
            ...
            Phpfox::getLib('mail')->to($aPoll['user_id'])
                ->subject(array('email_subject_your_poll_title_has_been_approved', array('title' => $aPoll['question'])))
                ...
                ->notification('poll.poll_notifications')
                ->send();
    ...
    public function addVote($iUser, $iPoll, $iAnswer)
    {
        ...
        $bIsSingleChoice = (!empty($iAnswer) && is_array($iAnswer) && count($iAnswer) < 2) || (!is_array($iAnswer) && (int)$iAnswer);
        $iAnswer = $bIsSingleChoice ? (is_array($iAnswer) ? $iAnswer[0] : $iAnswer) : $iAnswer;
