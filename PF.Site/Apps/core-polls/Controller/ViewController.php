...
class ViewController extends \Phpfox_Component
{
    ...
    public function process()
    {
        ...
        if ($aPoll === false) {
            return Phpfox_Error::display(_p('not_a_valid_poll'));
        }
        ...
        if (isset($aPoll['module_id']) && !empty($aPoll['item_id']) && !Phpfox::isModule($aPoll['module_id'])) {
            return Phpfox_Error::display(_p('Cannot find the parent item.'));
        }

        if (isset($aPoll['module_id']) && Phpfox::isModule($aPoll['module_id']) && Phpfox::hasCallback($aPoll['module_id'], 'checkPermission')) {
            if (!Phpfox::callback($aPoll['module_id'] . '.checkPermission', $aPoll['item_id'], 'poll.view_browse_polls')) {
        ...
        if (isset($aPoll['module_id']) && Phpfox::hasCallback($aPoll['module_id'], 'getItem')) {
            $aCallback = Phpfox::callback($aPoll['module_id'] . '.getItem', $aPoll['item_id']);
            if ($aCallback === false) {
                return Phpfox_Error::display(_p('Cannot find the parent item.'));
            }

            $this->template()
                ->setBreadCrumb(isset($aCallback['module_title']) ? $aCallback['module_title'] : _p($aPoll['module_id']), $this->url()->makeUrl($aPoll['module_id']))
                ->setBreadCrumb($aCallback['title'], Phpfox::permalink($aPoll['module_id'], $aPoll['item_id']))
                ->setBreadCrumb(_p('polls'), $this->url()->makeUrl($aPoll['module_id'], array($aPoll['item_id'], 'poll')));
        } else {
            $this->template()->setBreadCrumb(_p('polls'), $this->url()->makeUrl('poll'));
        }
