...
<form  method="post" action="{if $bIsCustom}#{else}{url link='current'}{if isset($aForms.poll_id)}id_{$aForms.poll_id}{/if}{/if}" name="js_poll_form" id="js_poll_form" onsubmit="{if $bIsCustom}if ({$sGetJsForm}) {l} $Core.poll.submitCustomPoll(this); {r} return false;{else}{$sGetJsForm}{/if}" {if Phpfox::getUserParam('poll.poll_can_upload_image')}enctype="multipart/form-data"{/if}>
	...
	{/if}
	{if $bIsCustom && empty($sModule)}
	    <div><input type="hidden" name="val[module_id]" value="{$sModuleId}"></div>
        <div><input type="hidden" name="val[is_custom]" value="1"></div>
    {/if}
    {if !empty($sModule)}
    ...
    <div class="form-group poll-app create votes">
        <div class="privacy-block-content">
            <div class="item_is_active_holder {if !isset($aForms.hide_vote)}item_selection_active{else}{if $aForms.hide_vote}item_selection_not_active{else}item_selection_active{/if}{/if}">
                <span class="js_item_active item_is_active"><input type="radio" name="val[hide_vote]" value="0" class="close_warning checkbox" style="vertical-align:middle;"{value type='checkbox' id='hide_vote' default='0' selected=true}/> {_p var='yes'}</span>
                <span class="js_item_active item_is_not_active"><input type="radio" name="val[hide_vote]" value="1" class="close_warning checkbox" style="vertical-align:middle;"{value type='checkbox' id='hide_vote' default='1'}/> {_p var='no'}</span>
            </div>
            <div class="inner">
    ...
    <div class="form-group poll-app create votes">
        <div class="privacy-block-content">
            <div class="item_is_active_holder">
                <span class="js_item_active item_is_active"><input type="radio" name="val[is_multiple]" value="1" class="close_warning v_middle"{value type='radio' id='is_multiple' default='1'}/> {_p var='yes'}</span>
                <span class="js_item_active item_is_not_active"><input type="radio" name="val[is_multiple]" value="0" class="close_warning v_middle"{value type='radio' id='is_multiple' default='0' selected='true'}/> {_p var='no'}</span>
            </div>
            <label>
      ...
      <div class="form-group poll-app create votes poll-end-time">
          <div class="privacy-block-content">
              <div class="item_is_active_holder">
                  <span class="js_item_active js_poll_expire item_is_active"><input type="radio" name="val[enable_close]" value="1" class="close_warning v_middle"{value type='radio' id='enable_close' default='1'}/> {_p var='yes'}</span>
                  <span class="js_item_active js_poll_expire item_is_not_active"><input type="radio" name="val[enable_close]" value="0" class="close_warning v_middle"{value type='radio' id='enable_close' default='0' selected='true'}/> {_p var='no'}</span>
              </div>
              <div class="inner">
    ...
    {if !$bIsCustom && empty($sModule) && Phpfox::isModule('privacy')}
        <div class="special_close_warning">
            <div class="form-group">
                <label>
                    {_p var='privacy'}:
