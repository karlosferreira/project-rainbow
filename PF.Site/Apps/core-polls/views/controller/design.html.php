	...
	<form action="{url link='poll.design' id=$aPoll.poll_id}" method="post" id="js_poll_design_form">
		...
		<ul id="js_poll_design_wrapper" class="dont-unbind-children poll-app clearfix">
			<li class="design-control">
				...
				<input type="hidden" name="val[js_poll_background]" value="{if $aPoll.background}{$aPoll.background}{else}ebebeb{/if}" data-rel="backgroundChooser" class="_colorpicker close_warning" />
				<div class="_colorpicker_holder"></div>
			</li>
			<li class="design-control">
				<b class="mr-1">{_p var='percent'}</b>
				<input type="hidden" name="val[js_poll_percentage]" value="{if $aPoll.percentage}{$aPoll.percentage}{else}297fc7{/if}" data-rel="percentageChooser" class="_colorpicker close_warning" />
				<div class="_colorpicker_holder"></div>
			</li>
			<li class="design-control">
				<b class="mr-1">{_p var='border'}</b>
				<input type="hidden" name="val[js_poll_border]" value="{if $aPoll.border}{$aPoll.border}{else}ffffff{/if}" data-rel="borderChooser" class="_colorpicker close_warning" />
				<div class="_colorpicker_holder"></div>
			</li>
		</ul>
	</form>
