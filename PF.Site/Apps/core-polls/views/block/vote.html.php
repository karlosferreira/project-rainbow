	                ...
	                {if $aPoll.canViewResultVote}
	                    {if count($aAnswer.some_votes) && (Phpfox::getUserParam('privacy.can_view_all_items') || ($aPoll.hide_vote == '1' && Phpfox::getUserId() == $aPoll.user_id) || $aPoll.hide_vote != '1')}
                            {foreach from=$aAnswer.some_votes item=aUserVote}
                                <div class="vote-member-inner">{img user=$aUserVote suffix='_120_square'}</div>
                            {/foreach}
...
<div id="vote_{$aPoll.poll_id}" class="clearfix mt-2"  {if isset($aPoll.voted) && Phpfox::getUserParam('poll.poll_can_change_own_vote')}style="display:none;"{/if}>
	<form class="poll_form" method="post" action="{url link='current'}" id="js_poll_form_{$aPoll.poll_id}">
		...
		<div class="mt-2">
			<button class="button btn-primary " type="button" onclick="$Core.poll.submitPoll({if !Phpfox::getUserParam('poll.poll_can_change_own_vote')}true{else}false{/if}, {$aPoll.poll_id});">{_p var='submit_your_vote'}</button>
			{if isset($aPoll.voted)}
