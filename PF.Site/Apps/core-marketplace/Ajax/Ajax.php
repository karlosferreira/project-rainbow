...
class Ajax extends Phpfox_Ajax
{
    public function canBuy()
    {
        ...
        $listingId = (int)$this->get('id');
        $invoiceId = (int)$this->get('invoice_id');

        if (!$invoiceId || !$listingId || empty($listing = Phpfox::getService('marketplace')->getDetail($listingId))) {
            return false;
        }

        if ((!empty($listing['is_sell']) || !empty($listing['allow_point_payment']))
            && (int)$listing['view_id'] != 2
            && Phpfox::getUserId() != $listingId['user_id']) {
            return $this->call('window.location.href= "' . Phpfox::getLib('url')->makeUrl('marketplace.purchase', ['invoice' => $invoiceId]) . '"');
        }

        return $this->alert(_p('marketplace_permission_denied'));
    }
