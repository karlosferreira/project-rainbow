...
<div class="main_break">
	...
	<form method="post" action="{url link='blog.add'}" id="core_js_blog_form" onsubmit="{$sGetJsForm}" enctype="multipart/form-data">
        ...
        {if Phpfox::isModule('tag') && Phpfox::getParam('tag.enable_tag_support')}
            {module name='tag.add' sType=blog bCloseWarning=true}
        {/if}

        <div class="special_close_warning">
            {if empty($sModule) && Phpfox::isModule('privacy')}
                <div class="form-group">
                    <label for="text">{_p var='blog_privacy'}</label>
                    {module name='privacy.form' privacy_name='privacy' privacy_info='control_who_can_see_this_blog' default_privacy='blog.default_privacy_setting'}
                </div>
            {/if}
        </div>
