...
class IndexController extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        if (defined('PHPFOX_IS_PAGES_VIEW') && PHPFOX_IS_PAGES_VIEW && defined('PHPFOX_PAGES_ITEM_TYPE') && $aParentModule) {
            ...
            if (Phpfox::hasCallback($sService, 'checkPermission') && !Phpfox::callback($sService . '.checkPermission', $aParentModule['item_id'], 'blog.view_browse_blogs')) {
            ...
            if (!defined('PHPFOX_TAG_PARENT_MODULE')) {
                define('PHPFOX_TAG_PARENT_MODULE', $sService);
            }
            ...
            $this->template()
                ->clearBreadCrumb()
                ->setBreadCrumb(Phpfox::getService($sService)->getTitle($aParentModule['item_id']), $aParentModule['url'])
                ->setBreadCrumb(_p('blogs'), $aParentModule['url'] . 'blog/');
        } elseif (Phpfox::getUserParam('blog.add_new_blog') && Phpfox::getService('blog')->checkLimitation() && (!defined('PHPFOX_IS_USER_PROFILE') || (defined('PHPFOX_IS_USER_PROFILE') && Phpfox::getUserId() == $aUser['user_id']))) {
        ...
        ->setPhrase([
            'are_you_sure_you_want_to_delete_this_blog_permanently',
        ]);
        ...
        if (Phpfox::getUserParam('blog.delete_user_blog') || $bIsAdmin) {
            $aModerationMenu[] = array(
                'phrase' => _p('delete'),
                'action' => 'delete',
                'message' => _p('are_you_sure_you_want_to_delete_selected_blogs_permanently')
            );
        }
