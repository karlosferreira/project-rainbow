...
class Process extends Phpfox_Service
{
    ...
    public function add($aVals)
    {
        ...
        if (!Phpfox::getService('ban')->checkAutomaticBan($aVals['text'] . ' ' . $aVals['title'])) {
            return false;
        }
    ...
    public function update($iId, $iUserId, $aVals, &$aRow = null)
    {
        ...
        // check if the user entered a forbidden word
        if (!Phpfox::getService('ban')->checkAutomaticBan($aVals['title'] . ' ' . $aVals['text'])) {
            return false;
        }
    ...
    public function approve($iId)
    {
        ...
        Phpfox::getLib('mail')->to($aBlog['user_id'])
            ...
            ->notification('blog.email_notification')
            ->send();
        //clear cache
        $this->cache()->remove();
        return true;
