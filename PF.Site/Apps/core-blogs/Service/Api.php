...
class Api extends ApiServiceBase
{
    ...
    public function get($params, $messages = [])
    {
        ...
        if (Phpfox::isModule('track')) {
            if (!Phpfox::getUserBy('is_invisible')) {
                if (!$aBlog['is_viewed']) {
                    $updateCounter = true;
                    Phpfox::getService('track.process')->add('blog', $aBlog['blog_id']);
                } else {
                    if (!Phpfox::getParam('track.unique_viewers_counter')) {
                        $updateCounter = true;
                        Phpfox::getService('track.process')->add('blog', $aBlog['blog_id']);
                    } else {
                        Phpfox::getService('track.process')->update('blog', $aBlog['blog_id']);
                    }
                }
            }
        } else {
        ...
        if ($updateCounter) {
            Phpfox::getService('blog.process')->updateView($aBlog['blog_id']);
        }
    ...
    public function post()
    {
        ...
        } elseif (!Phpfox::getService('blog')->checkLimitation()) {
            return $this->error(_p('blog_you_have_reached_your_limit_to_create_new_blog'));
        }
        ...
        if (Phpfox_Error::isPassed()) {
            ...
            foreach ($aItems as $aItem) {
                ...
                if (Phpfox::isModule('attachment')) {
                    $aAttachments = Phpfox::getService('attachment')->getForItemEdit($aItem['blog_id'], 'blog', null, true);
                    $aItem['attachments'] = [];
