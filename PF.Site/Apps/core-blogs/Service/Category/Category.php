...
class Category extends Core_Service_Systems_Category_Category
{
     ...
     * @return mixed
     */
    public function getBlogByCategoryForRssFeed(&$aFeed, &$sDescription)
    {
        $sCondition = 'AND blog.is_approved = 1 AND blog.privacy = 0 AND blog.post_status = 1';
        $sCondition .= Phpfox::getService('blog')->getConditionsForSettingPageGroup('blog');
        if (Phpfox::isAppActive('PHPfox_Groups')) {
            ...
            $sCondition .= ' AND blog.blog_id NOT IN (' . $aNotInclude . ')';
        }

        (($sPlugin = Phpfox_Plugin::get('blog.component_service_category_get_blog_by_category_rss')) ? eval($sPlugin) : false);

        list(, $aRows) = Phpfox::getService('blog.category')->getBlogsByCategory(Phpfox::getLib('phpfox.request')->get('category'), 0, [$sCondition], 'blog.time_stamp DESC', 0, Phpfox::getParam('rss.total_rss_display'));
     ...
     * @param $aRow
     */
    public function getForRssFeedGroup(&$aRow)
    {
        $aCategories = $this->getForBrowse();
        if (is_array($aCategories) && count($aCategories)) {
            foreach ($aCategories as $aCategory) {
