...
class Browse extends Phpfox_Service
{
    ...
    public function getQueryJoins($bIsCount = false, $bNoQueryFriend = false)
    {
        ...
        $aHiddenViews = Phpfox::getService('blog')->getHiddenViews();
        $sView = \Phpfox_Request::instance()->get('view');
        if (Phpfox::getParam('core.section_privacy_item_browsing') && (!empty($aHiddenViews) && !in_array($sView, $aHiddenViews)) && Phpfox::getLib('search')->isSearch())
        {
            db()->join(Phpfox::getT('blog_text'),'blog_text', 'blog_text.blog_id = blog.blog_id');
        }
