    ...
    public function process()
    {
        ...
        $aMetaKeys = [
            'photo_meta_description' => [
                'description' => '<title>Photo Meta Description</title><info>Meta description added to pages related to the Photo app. <a role="button" onclick="$Core.editMeta(\'seo_photo_meta_description\', true)">Click here</a> to edit meta description.<span style="float:right;">(SEO) <input style="width:150px;" readonly value="seo_photo_meta_description"></span></info>',
                'meta_value'  => 'Share your photos with friends, family, and the world on Site Name.'
            ],
            'photo_meta_keywords'    => [
                'description' => '<title>Photo Meta Keywords</title><info>Meta keywords that will be displayed on sections related to the Photo app. <a role="button" onclick="$Core.editMeta(\'seo_photo_meta_keywords\', true)">Click here</a> to edit meta keywords.<span style="float:right;">(SEO) <input style="width:150px;" readonly value="seo_photo_meta_keywords"></span></info>',
        ...
        foreach ($aMetaKeys as $sSetting => $aMetaKey) {
            ...
            if (!Lib::phrase()->isPhrase($sNewPhrase)) {
                ...
                } else if (preg_match('/\{phrase var=(.*)\}/i', $sValue, $aMatches)) {
                    Lib::phrase()->clonePhrase(trim($aMatches[1], '\'\"'), $sNewPhrase);
                } else {
