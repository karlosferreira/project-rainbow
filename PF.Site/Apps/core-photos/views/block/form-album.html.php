...
<div class="form-group">
    ...
    {else}
        <input class="form-control close_warning" required type="text" name="val[name]" id="name" value="{value type='input' id='name'}" size="30" maxlength="150" autofocus/>
    {/if}
</div>
<div class="form-group">
    <label for="description">{_p var='description'}</label>
    <textarea class="form-control close_warning" name="val[description]" id="description" cols="40" rows="5">{value type='textarea' id='description'}</textarea>
</div>
...
{else}
  {if Phpfox::isModule('privacy') && Phpfox::getUserParam('photo.can_use_privacy_settings')}
      <div class="form-group form-group-follow special_close_warning">
          ...
          {module name='privacy.form' privacy_name='privacy' privacy_info='photo.control_who_can_see_this_photo_album_and_any_photos_associated_with_it' privacy_custom_id='js_custom_privacy_input_holder_album' default_privacy='photo.default_album_privacy_setting'}
      </div>
  {/if}
{/if}