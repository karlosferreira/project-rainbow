...
<div class="go_left t_center" style="width:30%;">
    <a class="{if ($aPhoto.mature == 0 || (($aPhoto.mature == 1 || $aPhoto.mature == 2) && Phpfox::getUserId() && Phpfox::getUserParam('photo.photo_mature_age_limit') <= Phpfox::getUserBy('age'))) || $aPhoto.user_id == Phpfox::getUserId()}{else}photo-mature{/if}" href="{$aPhoto.link}"
       {if $aPhoto.mature == 1} onclick="tb_show('{_p('warning')}', $.ajaxBox('photo.warning', 'height=300&amp;width=350&amp;link={$aPhoto.link}')); return false;"{else} title="{_p var='title_by_full_name' title=$aPhoto.title|clean full_name=$aPhoto.full_name|clean}"{/if}>
