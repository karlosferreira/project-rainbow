<div class="item-inner">
    <div class="photo_edit_wrapper">
        {if count($aAlbums)}
            <div class="photo_edit_holder dropdown {if $aForms.album_id > 0}success{/if}">
                ...
                <div class="form-group dropdown-menu pf-dropdown-not-hide-photo">
                    ...
                    <select id="{$aForms.photo_id}" name="val[{$aForms.photo_id}][move_to]" class="form-control close_warning" onchange="$Core.Photo.toggleEditAction(this,'album');" data-album_id="{$aForms.album_id}">
        ...
        <div class="form-group item-mature hide dropdown">
            ...
            <div class="dropdown-menu pf-dropdown-not-hide-photo">
                ...
                <div class="disable-download">
                    <label>
                        <input type="checkbox" class="close_warning" name="val[{$aForms.photo_id}][allow_download]" value="1" {value type='checkbox' id='allow_download' default=1} onclick="$Core.Photo.toggleEditAction(this,'download');"/>
        ...
        {if ($aForms.album_id == '0' || $aForms.type_id == 1) && $aForms.group_id == '0' && Phpfox::isModule('privacy')}
        <div class="photo_edit_input">
            <div class="form-group form-group-follow special_close_warning">
    ...
    <div class="form-group item-title page hide">
        <input placeholder="{_p var='Title'}" title="{_p var='Title'}" id="title" type="{if Phpfox::getParam('photo.photo_show_title')}text{else}hidden{/if}" name="val{if isset($aForms.photo_id)}[{$aForms.photo_id}]{/if}[title]" value="{if isset($aForms.title)}{$aForms.title|clean}{else}{value type='input' id='title'}{/if}" size="30" maxlength="150" onfocus="this.select();" class="form-control special_close_warning" required/>
    </div>

    <div class="form-group item-description page hide">
        <textarea title="{_p var='description'}" rows="4" id="{if isset($aForms.photo_id)}{$aForms.photo_id}{/if}description" placeholder="{_p var='Description'}" name="val{if isset($aForms.photo_id)}[{$aForms.photo_id}]{/if}[description]" class="form-control close_warning">{if isset($aForms.description)}{$aForms.description|clean}{else}{value type='input' id='description'}{/if}</textarea>
    </div>
    {if Phpfox::isModule('tag') && Phpfox::getParam('tag.enable_tag_support')}
    <div class="form-group item-topic page hide">
        <input type="text" class="topic close_warning" name="val{if $aForms.photo_id}[{$aForms.photo_id}]{/if}[tag_list]" value="{value type='input' id='tag_list'}" size="30" placeholder="{_p var='topics_separate_multiple_topics_with_commas'}" title="{_p var='topics_separate_multiple_topics_with_commas'}"/>
    </div>
    {/if}
