...
class Process extends Phpfox_Service
{
    ...
    {
        $aAlbum = db()->select('album_id, user_id, module_id, group_id, is_sponsor, is_featured')
            ->from($this->_sTable)
    ...
    {
        $aAlbum = db()->select('album_id, privacy, privacy_comment, timeline_id')
            ->from($this->_sTable)
            ->where('album_id = ' . (int)$iAlbumId)
            ->execute('getSlaveRow');

        if (empty($aAlbum['album_id']) || !empty($aAlbum['timeline_id'])) {
            return false;
        }
        if ($iPrivacy === null) {
