...
class Process extends Phpfox_Service
{
    ...
    public function add($aVals)
    {
        ...
        if ((Phpfox::getUserParam('photo.can_tag_own_photo') && $aPhoto['user_id'] == Phpfox::getUserId()) || Phpfox::getUserParam('photo.can_tag_other_photos')) {
            ...
            if (Phpfox::isModule('feed')) {
                ...
                if ($aFeed) {
                    ...
                    $iFeedId = db()->select('feed_id')
                        ->from(':photo_feed')
                        ->where('photo_id = ' . $aVals['item_id'] . ' AND feed_time = 0')
                        ->execute('getField');
                }
                if (!empty($iFeedId)) {
            ...
            $sLink = Phpfox::getLib('url')->permalink('photo', $aPhoto['photo_id'], $bCanUseTitle ? $aPhoto['title'] : null);
            ...
            Phpfox::getLib('mail')->to($iTagUserId)
                ...
                ->message((Phpfox::getUserId() == $aPhoto['user_id'] ? [
                    $bCanUseTitle ? 'full_name_tagged_you_on_gender_photo' : 'photo_full_name_tagged_you_on_gender_photo', [
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender'    => Phpfox::getService('user')->gender(Phpfox::getUserBy('gender'), 1),
                        'link'      => $sLink,
                        'title'     => $bCanUseTitle ? $aPhoto['title'] : _p('photo_photo_number', ['number' => $aPhoto['photo_id']])
                    ]
                ] : [
                    $bCanUseTitle ? 'full_name_tagged_you_on_user_photo' : 'photo_full_name_tagged_you_on_user_photo', [
                        'full_name'       => Phpfox::getUserBy('full_name'),
    ...
    public function delete($iId)
    {
        ...
        if (($aTag['post_user_id'] == Phpfox::getUserId() || $aTag['photo_owner_id'] == Phpfox::getUserId() || $aTag['tag_user_id'] == Phpfox::getUserId())) {
            ...
            if (Phpfox::isModule('feed')) {
                ...
                Phpfox::getService('feed.tag')->deleteTaggedUser($aTag['photo_id'], $aTag['tag_user_id'], 'photo');
            }
