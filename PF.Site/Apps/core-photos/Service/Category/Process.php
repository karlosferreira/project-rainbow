...
class Process extends \Core_Service_Systems_Category_Process
{
    ...
    {
        db()->update($this->_sTable, ['used' => ['= used +', 1]], 'category_id = ' . (int)$iCategory);

        // Add the category data
        return db()->insert($this->_sTableData, [
                'photo_id'    => (int)$iPhoto,
    ...
    public function deleteCategory($iCategoryId, $aVals = [])
    {
            ...
            switch ($aVals['delete_type']) {
                ...
                case 2:
                    if (!empty($aVals['new_category_id'])) {
                        ...
                        db()->update($this->_sTable, ['parent_id' => $aVals['new_category_id']],
        ...
        $this->cache()->removeGroup('photo_category');
