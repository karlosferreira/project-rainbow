...
class Storage extends \OAuth2\Storage\Pdo {
    ...
    public function deleteExpiredAccessToken() {
        $expires = date('Y-m-d H:i:s', PHPFOX_TIME);
        db()->delete($this->config['refresh_token_table'], "expires < '" . $expires . "'");
        return db()->delete($this->config['access_token_table'], "expires < '" . $expires . "'");
    }
    ...
    {
        list($result, $user) = \Phpfox::getService('user.auth')->login($user['email'], $password);
        $this->_user = $user;
        return $result;
    }
