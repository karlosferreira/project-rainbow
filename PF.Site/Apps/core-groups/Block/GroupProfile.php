    ...
    public function process()
    {
        ...
        $this->template()->assign([
                'sHeader' => '<a href="' . $this->url()->makeUrl($aUser['user_name'],
                        'groups/?view=all') . '" title="' . ($aUser['user_id'] == Phpfox::getUserId() ? _p('Groups you created and joined') : _p('groups_full_name_created_and_joined', ['full_name' => Phpfox::getLib('parse.output')->clean($aUser['full_name'])])) . '">' . _p('joined_groups') . '<span>' . $iTotal . '</span></a>',
                'aPagesList' => $aPages,
