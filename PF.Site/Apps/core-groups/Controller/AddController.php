...
class AddController extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        if(!($iEditId = $this->request()->getInt('id'))) {
            Phpfox::getService('groups')->canUserCreateNewGroup();
        }
        ...
        if ($iEditId && ($aPage = Phpfox::getService('groups')->getForEdit($iEditId))) {
            ...
            if (($subTab = $this->request()->get('sub_tab'))
                ...
                && !empty($groupActiveTab = $this->template()->getVar('sActiveTab'))
                && in_array($groupActiveTab, array_keys($aMenus))) {
            ...
            if (($aVals = $this->request()->getArray('val'))) {
                if (Phpfox::getService('groups.process')->update($aPage['page_id'], $aVals, $aPage)) {
                    ...
                    // update old group
                    $this->url()->send('groups.add', ['id' => $aPage['page_id'], 'tab' => empty($aVals['current_tab']) ? '' : $aVals['current_tab']], _p(defined('PHPFOX_UPLOAD_GROUPS_PHOTO_IS_PENDING') ? 'group_successfully_updated_the_profile_photo_will_be_updated_after_approval_process_is_done' : 'group_successfully_updated'));
                } else {
