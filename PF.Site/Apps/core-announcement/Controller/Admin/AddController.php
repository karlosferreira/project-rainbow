...
use Phpfox;
use Phpfox_Plugin;
use Apps\Core_Announcement\Service\Process;
    ...
    public function process()
    {
        ...
        if ($iEditId = $this->request()->getInt('announcement_id')) {
            if ($aAnnouncement = Phpfox::getService('announcement')->getAnnouncementById($iEditId)) {
                if ($aAnnouncement['gender'] == Process::CUSTOM_GENDER_VALUE) {
                    $customGenders = !empty($aAnnouncement['custom_gender']) ? unserialize($aAnnouncement['custom_gender']) : [];
                    $this->template()->setHeader('cache', [
                        '<script>aUserGenderCustom = ' . json_encode($customGenders) . '; bIsCustomGender = true;</script>'
                    ]);
                }
                ...
                $this->setParam(array(
                        'country_child_value' => $aAnnouncement['country_iso'],
                    )
                );
                $bIsEdit = true;
            }
