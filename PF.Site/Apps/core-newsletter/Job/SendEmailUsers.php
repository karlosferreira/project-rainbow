...
class SendEmailUsers extends JobAbstract
{
    ...
    public function perform()
    {
        ...
        $sWhere .= ' AND u.profile_page_id = 0';
        // filter the audience
        ...
        $aUsers = db()->select($sSelect)
            ...
            ->leftJoin(Phpfox::getT('user_notification'), 'un', 'un.user_id = u.user_id AND (un.user_notification IN ("' . implode('","', ['newsletter.can_receive_notification', 'core.enable_notifications']) . '"))')
            ->where($sWhere)
            ->order('u.user_id')
        ...
        foreach ($aUsers as $aUser) {
            ...
            $aTemp['subject'] = htmlspecialchars_decode($aTemp['subject']);
