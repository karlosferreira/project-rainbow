...
{if isset($bIsEditAlbum) && $bIsEditAlbum && !isset($bIsUploaded)}
    <div id="js_music_upload_song" class="music-share-song">
        <div class="form-group js_core_init_selectize_form_group">
            ...
            <select class="form-control close_warning" placeholder="{_p var='select_a_genre_dot'}" multiple="multiple" name="val[genre][]">
            ...
            <div class="js_music_form_holder" {if !$bIsEdit}style="display: none;"{/if}>
                ...
                <div class="music-app info-each-item">
                    <div class="form-group" {if $bIsEdit && !$aForms.album_id && !$bCanSelectAlbum}style="display:none;"{/if}>
                        ...
                        <select name="{if !$bIsEdit}val[{$aForms.song_id}][album_id]{else}val[album_id]{/if}" id="js_music_album_select" class="form-control close_warning" {if isset($bCanSelectAlbum) && !$bCanSelectAlbum}disabled{/if} onchange="if (empty(this.value)) {l} $(this).closest('.js_music_form_holder').find('.js_song_privacy_holder').slideDown(); {r} else {l} $(this).closest('.js_music_form_holder').find('.js_song_privacy_holder').slideUp(); {r}">
                    ...
                    <div class="form-group song_name close_warning">
                    ...
                    <div class="_form_extra">
                        {if !isset($bIsEditAlbum)}
                            <div class="form-group" style="display:none;">
                                ...
                                {if isset($aUploadAlbums) && count($aUploadAlbums)}
                                    <select class="form-control close_warning" name="val[{$aForms.song_id}][album_id]" id="music_album_id_select" onchange="if (empty(this.value)) {l} $('#js_song_privacy_holder').slideDown(); {r} else {l} $('#js_song_privacy_holder').slideUp(); {r}">
                        ...
                        <div class="form-group song_name js_core_init_selectize_form_group">
                            ...
                            <select class="form-control close_warning" placeholder="{_p var='select_a_genre_dot'}" multiple="multiple" name="{if !$bIsEdit}val[{$aForms.song_id}][genre][]{else}val[genre][]{/if}">
                                {foreach from=$aGenres item=aGenre}
                        ...
                        <div class="form-group special_close_warning" id="js_upload_photo_section_{$aForms.song_id}">
                        ...
                        {else}
                            ...
                            <div id="js_song_privacy_holder" class="js_song_privacy_holder" {if $aForms.album_id > 0}style="display:none"{/if}>
                                <div class="form-group special_close_warning">
                                    ...
                                    {if !$bIsEdit}
                                        {module name='privacy.form' privacy_name='privacy' privacy_array=$aForms.song_id privacy_info='music.control_who_can_see_this_song' default_privacy='music.default_privacy_setting_song' privacy_custom_id='js_custom_privacy_input_holder_'$aForms.song_id''}
                                    {else}
                                        {module name='privacy.form' privacy_name='privacy' privacy_info='music.control_who_can_see_this_song' default_privacy='music.default_privacy_setting_song'}
                                    {/if}
                                </div>
                            </div>
                            {/if}
