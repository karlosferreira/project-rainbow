...
class AlbumController extends \Phpfox_Component
{
    ...
    public function process()
    {
        ...
        $bIsPopup = $this->getParam('popup', false);
        $aCallback = false;
                        ...
                        switch ($sAction) {
                            case 'track':
                                $this->url()->permalink('music.album', $aAlbum['album_id'], $aAlbum['name'], true,
                                    _p('music_songs_successfully_uploaded') . (Phpfox::getUserParam('music.music_song_approval') ? ' ' . _p('note_that_it_will_have_to_be_approved_first_before_it_is_displayed_publicly') : ''));
                                break;
                            default:
                                $this->url()->permalink('music.album', $aAlbum['album_id'], $aAlbum['name'], true,
        ...
        if ($bIsEdit) {
            ...
            if (Phpfox::getService('music')->canUploadNewSong(Phpfox::getUserId(), false) && $aAlbum['user_id'] == Phpfox::getUserId()) {
