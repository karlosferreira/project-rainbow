...
class Process extends \Phpfox_Service
{
    ...
    public function answerQuiz($sQuiz, $aAnswers)
    {
        ...
        foreach ($aAnswers as $iQuestion => $iAnswer) {
            ...
            $this->database()->insert(Phpfox::getT('quiz_result'), [
                    'quiz_id' => $quizId,
        ...
        db()->updateCounter('quiz', 'total_play', 'quiz_id', $quizId);

        if (Phpfox::isModule('notification') && $quizOwnerId != $iUser) {
            $quizLink = Phpfox::permalink('quiz', $quizId, $quizTitle);
            Phpfox::getLib('mail')->to($quizOwnerId)
                ->subject(['quiz_full_name_played_your_quiz', ['full_name' => Phpfox::getUserBy('full_name')]])
                ->message(['quiz_full_name_played_on_your_quiz_question',
                    [
                        'link' => '<a href="' . $quizLink . '">' . $quizLink . '</a>',
                        'title' => $quizTitle,
                        'full_name' => Phpfox::getUserBy('full_name'),
                    ]
                ])
                ->notification('quiz.quiz_notifications')
                ->send();
            Phpfox::getService('notification.process')->add('quiz_answer', $quizId, $quizOwnerId, $iUser);
        }
    ...
    public function approveQuiz($iQuiz)
    {
        ...
        $bUpdate = $this->database()->update($this->_sTable, ['view_id' => '0', 'time_stamp' => PHPFOX_TIME],
            'quiz_id = ' . (int)$iQuiz) == 1;

        // check if it had been added before
        if (Phpfox::isModule('feed') && Phpfox::getParam('quiz.quiz_allow_create_feed_when_add_new_item', 1)) {
            if (!empty($aQuiz['module_id']) && !empty($aQuiz['item_id']) && Phpfox::hasCallback($aQuiz['module_id'], 'getFeedDetails')) {
                Phpfox::getService('feed.process')->callback(Phpfox::callback($aQuiz['module_id'] . '.getFeedDetails',
                    $aQuiz['item_id']))->add('quiz', $iQuiz, $aQuiz['privacy'], $aQuiz['privacy_comment'],
                    $aQuiz['item_id'], $aQuiz['user_id']);
            } elseif (empty($aQuiz['module_id']) || $aQuiz['module_id'] == 'quiz') {
        ...
        Phpfox::getLib('mail')->to($aQuiz['user_id'])
            ->subject(['email_subject_your_quiz_title_has_been_approved', ['title' => $aQuiz['title']]])
            ->message(['your_quiz_a_href_link_title_a_has_been_approved_to_view_this_quiz_follow_the_link_below_a_href_link_link_a',
                ['link' => $sLink, 'title' => $aQuiz['title']]])
            ->notification('quiz.quiz_notifications')
            ->send();
    ...
    public function update($aQuiz, $iUser)
    {
        ...
        if ($bHasAttachments) {
            Phpfox::getService('attachment.process')->updateItemId($aQuiz['attachment'], Phpfox::getUserId(), $aQuiz['quiz_id']);
        }
        ...
        $aUpdate = [
            ...
            'total_attachment' => (Phpfox::isModule('attachment') ? Phpfox::getService('attachment')->getCountForItem($aQuiz['quiz_id'], 'quiz') : '0')
        ];
        ...
        (empty($aVals['module_id']) && Phpfox::isModule('feed') ? Phpfox::getService('feed.process')->update('quiz', $aQuiz['quiz_id'],
        ...
        if (Phpfox::isModule('privacy')) {
            if ($aQuiz['privacy'] == '4') {
                Phpfox::getService('privacy.process')->update('quiz', $aQuiz['quiz_id'],
                    (isset($aQuiz['privacy_list']) ? $aQuiz['privacy_list'] : []));
            } else {
                Phpfox::getService('privacy.process')->delete('quiz', $aQuiz['quiz_id']);
            }
        }

        Phpfox::isModule('feed') && Phpfox::getService('feed.process')->clearCache('quiz', $aQuiz['quiz_id']);
        ...
        if (!empty($aQuiz['image_path'])) {
            ...
            if (Phpfox::getService('user.file')->remove($aParams)) {
                if (!isset($bSkipDefaultReturn)) {
                    $this->database()->update(Phpfox::getT('quiz'), ['image_path' => null, 'server_id' => 0],
                        'quiz_id = ' . $iQuiz);
                }
            } else {
                return false;
    ...
    public function deleteQuiz($iQuiz, $iUser)
    {
        ...
        // Delete attachment
        (Phpfox::isModule('attachment') ? Phpfox::getService('attachment.process')->deleteForItem($iUserId, $iQuiz, 'quiz') : null);

        if (Phpfox::isModule('feed')) {
            $feedProcess = Phpfox::getService('feed.process');
            $feedProcess->delete('quiz', $iQuiz);
            $feedProcess->delete('comment_quiz', $iQuiz);
            $feedProcess->delete('quiz', $iQuiz, 0, true);
        }

        (Phpfox::isModule('comment') ? Phpfox::getService('comment.process')->deleteForItem($iUserId, $iQuiz, 'quiz') : null);
    ...
    public function add(&$aVals, $iUser)
    {
        ...
        if (isset($aVals['item_id']) && isset($aVals['module_id'])) {
            ...
            $aInsert['module_id'] = Phpfox::getLib('parse.input')->clean($aVals['module_id']);
        }
        ...
        foreach ($aVals['q'] as $aQuestions) {
            ...
            $iQuestionId = $this->database()->insert(Phpfox::getT('quiz_question'), [
                    'quiz_id' => $iQuizId,
            ...
            foreach ($aQuestions['answers'] as $aAnswer) {
                $this->database()->insert(Phpfox::getT('quiz_answer'), [
                        'question_id' => $iQuestionId,
                        'answer' => Phpfox::getLib('parse.input')->clean($aAnswer['answer'], 255),
        ...
        if (!Phpfox::getUserParam('quiz.new_quizzes_need_moderation')) {
            if(empty($aVals['module_id']) && Phpfox::isModule('feed') && Phpfox::getParam('quiz.quiz_allow_create_feed_when_add_new_item', 1)) {
                Phpfox::getService('feed.process')->add('quiz', $iQuizId, $aVals['privacy'],
                    (isset($aVals['privacy_comment']) ? (int)$aVals['privacy_comment'] : 0));
            } elseif (!empty($aVals['item_id'])) {
                if (Phpfox::isModule('feed') && Phpfox::getParam('quiz.quiz_allow_create_feed_when_add_new_item', 1) && Phpfox::hasCallback($aVals['module_id'],'getFeedDetails')) {
                    Phpfox::getService('feed.process')
                        ->callback(Phpfox::callback($aVals['module_id'] . '.getFeedDetails', $aVals['item_id']))
                        ->add('quiz', $iQuizId, $aVals['privacy'], (isset($aVals['privacy_comment']) ? (int)$aVals['privacy_comment'] : 0), $aVals['item_id']);

                }

                //support add notification for parent module
                if (Phpfox::isModule('notification') && $aVals['module_id'] != '' &&
                    Phpfox::isModule($aVals['module_id']) && Phpfox::hasCallback($aVals['module_id'], 'addItemNotification')
                ) {
                    Phpfox::callback($aVals['module_id'] . '.addItemNotification', [
                        'page_id' => $aVals['item_id'],
                        'item_perm' => 'quiz.view_browse_quizzes',
                        'item_type' => 'quiz',
                        'item_id' => $iQuizId,
                        'owner_id' => Phpfox::getUserId(),
                        'items_phrase' => 'quizzes'
                    ]);
                }
            }
    ...
    public function sponsor($iId, $sType)
    {
        ...
        db()->update($this->_sTable, ['is_sponsor' => $iType], 'quiz_id = ' . (int)$iId);
