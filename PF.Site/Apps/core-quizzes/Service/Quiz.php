...
class Quiz extends \Phpfox_Service
{
    ...
    public function checkLimitation($userId = null)
    {
        ...
        $limit = trim(Phpfox::getUserParam('quiz.quiz_total_items_can_create'));
    ...
    public function checkStructure($aQuestions)
    {
        ...
        foreach ($aQuestions as &$aQuestion) {
            ...
            foreach ($aQuestion['answers'] as &$aAnswer) {
                ...
                if (($aAnswer['answer'] == '')) {
                    $aError[2] = _p('we_do_not_allow_empty_answers');
                }
            ...
            if (empty($aQuestion['question'])) {
                $aError[0] = _p('the_question_field_cannot_be_empty');
            }
        } // validate questions
    ...
    public function getQuizByUrl($iQuizId, $iUser = false, $bIsTakingQuiz = false, $iLimit = 5, $iPage = 1)
    {
        ...
        if (Phpfox::isModule('track')) {
            $sJoinQuery = $bIsUser ? 'track.user_id = ' . $iUserId : 'track.ip_address = \'' . $this->database()->escape($sIp) . '\'';
        ...
        if (Phpfox::isModule('friend')) {
            $this->database()->select('f.friend_id AS is_friend, ')->leftJoin(Phpfox::getT('friend'), 'f',
                "f.user_id = q.user_id AND f.friend_user_id = " . $iUserId);
        } else {
        ...
        if (Phpfox::isModule('like')) {
            $this->database()->select('l.like_id AS is_liked, ')
                ->leftJoin(Phpfox::getT('like'), 'l',
                    'l.type_id = \'quiz\' AND l.item_id = q.quiz_id AND l.user_id = ' . $iUserId);
        }
        ...
        if ($aQuiz['view_id'] == '1' && !$bCanApprove && $aQuiz['user_id'] != $iUserId) {
            return false;
        }
    ...
    public function getConditionsForSettingPageGroup($sPrefix = 'q')
    {
        $aModules = [];
        // Apply settings show quiz of pages / groups
        ...
        if (count($aModules)) {
            return ' AND (' . $sPrefix . '.module_id IN (\'' . implode('\',\'',
                    $aModules) . '\') OR ' . $sPrefix . '.module_id = \'quiz\')';
        } else {
            return ' AND ' . $sPrefix . '.module_id = \'quiz\'';
        }
    }
    ...
    public function getFeatured($iLimit = 4, $iCacheTime = 5)
    {
        ...
        if (($sQuizIds = $this->cache()->get($sCacheId, $iCacheTime)) === false) {
            ...
            $aQuizIds = $this->database()->select('q.quiz_id')
                ->from($this->_sTable, 'q')
                ->join(Phpfox::getT('user'), 'u', 'u.user_id = q.user_id')
                ->where('q.view_id = 0 AND q.is_featured = 1' . $sExtraCond)
                ->order('rand()')
    ...
    public function getSponsored($iLimit = 4, $iCacheTime = 5)
    {
        ...
        if (($sQuizIds = $this->cache()->get($sCacheId, $iCacheTime)) === false) {
            ...
            $aQuizIds = $this->database()->select('q.quiz_id')
                ...
                ->where('q.view_id = 0 AND q.is_sponsor = 1 AND s.module_id = \'quiz\' AND s.is_active = 1 AND s.is_custom = 3' . $sExtraCond)
                ->order('rand()')
                ->limit(Phpfox::getParam('core.cache_total'))
                ->execute('getSlaveRows');
            foreach ($aQuizIds as $key => $aId) {
    ...
    public function addExtraInformation(&$aQuiz) {
        $aQuiz['link'] = \Phpfox_Url::instance()->permalink('quiz', $aQuiz['quiz_id'], $aQuiz['title']);
        if (isset($aQuiz['aTakenBy'])) {
            $aQuiz['taken_by'] = [];
            foreach ($aQuiz['aTakenBy'] as $key => $value) {
                $aQuiz['taken_by'][] = $value;
            }
        }
        if (!empty($aQuiz['image_path'])) {
            $aQuiz['image_path'] = Phpfox::getLib('image.helper')->display([
        ...
        foreach ($aQuestions as $iKey => $aQuestion) {
            $aQuestionItems[$iKey] = [
                'quiz_id'       => $aQuestion['quiz_id'],
                'question_id'   => $aQuestion['question_id'],
                'question'      => $aQuestion['question'],
            ];
            $aAnswers = $this->database()->select('*')
                ->from(Phpfox::getT('quiz_answer'))
                ->where(['question_id' => $aQuestion['question_id']])
                ->execute('getSlaveRows');
            $aAnswerItems = [];
            foreach ($aAnswers as $aAnswer) {
                $aAnswerItems[] = [
                    'answer_id'     => $aAnswer['answer_id'],
                    'answer'        => $aAnswer['answer'],
                    'is_correct'    => $aAnswer['is_correct'],
                ];
            }
            $aQuestionItems[$iKey]['answers'] = $aAnswerItems;
        }

        $aQuiz['questions'] = $aQuestionItems;
    }
    /**
