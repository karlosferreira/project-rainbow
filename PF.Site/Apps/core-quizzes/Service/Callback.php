...
class Callback extends \Phpfox_Service
{
    ...
    public function getNewsFeed($aRow)
    {
        ...
        $aRow['text'] = _p('a_href_user_link_full_name_a_added_a_new_quiz_a_href_question_url_question_a', [
    ...
    public function getActivityFeedComment($aRow)
    {
        ...
        $aItem = $this->database()->select('b.quiz_id, b.title, b.module_id, b.item_id, b.time_stamp, b.total_comment, b.total_like, c.total_like, ct.text_parsed AS text, ' . Phpfox::getUserField())
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        Phpfox::getService('comment.process')->notify([
                ...
                'owner_subject' => ['full_name_commented_on_one_of_your_quiz_title',
                    ['full_name' => Phpfox::getUserBy('full_name'), 'title' => $aQuiz['title'], 'link' => $sLink]],
                'owner_message' => ['full_name_commented_on_your_quiz_a_href',
                    ['full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'title' => $aQuiz['title']]],
                'owner_notification' => 'comment.add_new_comment',
                'notify_id' => 'comment_quiz',
                'mass_id' => 'quiz',
                'mass_subject' => (Phpfox::getUserId() == $aQuiz['user_id'] ? ['full_name_commented_on_gender_quiz',
                    [
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aQuiz['gender'], 1)
                    ]]
                    : ['full_name_commented_on_other',
                        ['full_name' => Phpfox::getUserBy('full_name'), 'other_full_name' => $aQuiz['full_name']]])
                ,
                'mass_message' => (Phpfox::getUserId() == $aQuiz['user_id'] ?
                    ['full_name_commented_on_gender_quiz', [
                        'full_name' => Phpfox::getUserBy('full_name'),
                        'gender' => Phpfox::getService('user')->gender($aQuiz['gender'], 1),
                        'title' => $aQuiz['title'],
                        'link' => $sLink
                    ]]
                    : ['full_name_commented_on_other_full_name_s_quiz', [
                        'full_name' => Phpfox::getUserBy('full_name'),
    ...
    public function getCommentNotification($aNotification)
    {
        ...
        if ($aNotification['user_id'] == $aRow['user_id'] && !isset($aNotification['extra_users'])) {
            $sPhrase = _p('user_names_commented_on_quiz', [
            ...
            $sPhrase = _p('user_names_commented_on_your', [
                'user_names' => Phpfox::getService('notification')->getUsers($aNotification),
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['title'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
            ]);
        } else {
            $sPhrase = _p('user_names_commented_full_name', [
                'user_names' => Phpfox::getService('notification')->getUsers($aNotification),
    ...
    public function getItemName($iId, $sName)
    {
        return '<a href="' . Phpfox_Url::instance()->makeUrl('comment.view',
                ['id' => $iId]) . '">' . _p('on_name_s_quiz', ['name' => $sName]) . '</a>';
    }
    ...
    public function onDeleteUser($iUser)
    {
        ...
        $aAwnseredByUsers = db()->select('qr.quiz_id')
                                ->from(':quiz_result', 'qr')
                                ->join(':quiz', 'q', 'q.quiz_id = qr.quiz_id')
                                ->where([
                                    'qr.user_id' => (int)$iUser
                                ])->executeRows(false);
        if (!empty($aAwnseredByUsers)) {
            $quizIds = array_column($aAwnseredByUsers, 'quiz_id');
            db()->delete(':quiz_result', [
                'quiz_id' => ['in' => implode(',', $quizIds)],
                'user_id' => (int)$iUser,
            ]);
            foreach ($quizIds as $quizId) {
                db()->updateCounter('quiz', 'total_play', 'quiz_id', $quizId, true);
            }
        }
    }
    ...
    public function getNewsFeedFeedLike($aRow)
    {
        if ($aRow['owner_user_id'] == $aRow['viewer_user_id']) {
            $aRow['text'] = _p('a_href_user_link_full_name_a_liked_their_own_a_href_link_quiz_a', [
    ...
    {
        return [
            'message' => _p('a_href_user_link_full_name_a_likes_your_a_href_link_quiz_a', [
                    'full_name' => Phpfox::getLib('parse.output')->clean($aRow['full_name']),
                    'user_link' => Phpfox_Url::instance()->makeUrl($aRow['user_name']),
                    'link' => Phpfox_Url::instance()->makeUrl('quiz', ['redirect' => $aRow['item_id']])
                ]
            ),
            'link' => Phpfox_Url::instance()->makeUrl('quiz', ['redirect' => $aRow['item_id']])
        ];
    ...
    {
        return _p('a_href_user_link_full_name_a_likes_your_a_href_link_quiz_a', [
                'full_name' => Phpfox::getLib('parse.output')->clean(Phpfox::getUserBy('full_name')),
                'user_link' => Phpfox_Url::instance()->makeUrl(Phpfox::getUserBy('user_name')),
                'link' => Phpfox_Url::instance()->makeUrl('quiz', ['redirect' => $iItemId])
            ]
        );
    }
    ...
    public function pendingApproval()
    {
        $aPending[] = [
            ...
            'link' => Phpfox_Url::instance()->makeUrl('quiz', ['view' => 'pending'])
        ];
    ...
    public function getNotificationLike($aNotification)
    {
        ...
        if ($aNotification['user_id'] == $aRow['user_id']) {
            $sPhrase = _p('user_name_liked_gender_own_quiz_title', [
            ...
            $sPhrase = _p('user_name_liked_your_quiz_title', [
                'user_name' => Phpfox::getService('notification')->getUsers($aNotification),
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['title'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
            ]);
        } else {
            $sPhrase = _p('user_name_liked_span_class_drop_data_user_full_name_s_span_quiz_title', [
                'user_name' => Phpfox::getService('notification')->getUsers($aNotification),
                'full_name' => $aRow['full_name'],
                'title' => Phpfox::getLib('parse.output')->shorten($aRow['title'],
                    Phpfox::getParam('notification.total_notification_title_length'), '...')
    ...
    public function getNotificationApproved($aNotification)
    {
        ...
        $sPhrase = _p('your_quiz_title_has_been_approved', [
            'title' => Phpfox::getLib('parse.output')->shorten($aRow['title'],
                Phpfox::getParam('notification.total_notification_title_length'), '...')
        ]);
    ...
    public function getActivityFeed($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        if ($aRow['module_id'] && Phpfox::hasCallback($aRow['module_id'], 'checkPermission') && !Phpfox::callback($aRow['module_id'] . '.checkPermission', $aRow['item_id'], 'quiz.view_browse_quizzes')) {
            return false;
        }
    ...
    public function getCommentNotificationTag($aNotification)
    {
        ...
        $sPhrase = _p('user_name_tagged_you_in_a_comment_in_a_quiz', ['user_name' => $aRow['full_name']]);
    ...
    public function getToSponsorInfo($iId)
    {
        ...
        if (empty($aQuiz)) {
            return ['error' => _p('sponsor_error_quiz_not_found')];
        }

        $aQuiz['title'] = _p('quiz_sponsor_item_title', ['sQuizTitle' => $aQuiz['title']]);
        $aQuiz['paypal_msg'] = _p('quiz_sponsor_paypal_message_title', ['sQuizTitle' => $aQuiz['title']]);
    ...
    {
        return [
            'quiz.share_quizzes' => _p('who_can_share_quizzes'),
            'quiz.view_browse_quizzes' => _p('who_can_view_quizzes')
        ];
    }
    ...
    {
        if (!Phpfox::getService('pages')->hasPerm($aPage['page_id'], 'quiz.view_browse_quizzes') || !Phpfox::getUserParam('quiz.can_access_quiz')) {
            return null;
        }

        $aMenus[] = [
            'phrase' => _p('quizzes'),
            'url' => Phpfox::getService('pages')->getUrl($aPage['page_id'], $aPage['title'], $aPage['vanity_url']) . 'quiz/',
            'icon' => 'module/core-quiz.png',
            'landing' => 'quiz'
        ];
    ...
    {
        if (!Phpfox::getService('pages')->hasPerm($aPage['page_id'], 'quiz.share_quizzes')
            || !Phpfox::getUserParam('quiz.can_create_quiz')
            || !Phpfox::getUserParam('quiz.can_access_quiz')
            || !Phpfox::getService('quiz')->checkLimitation()
        ) {
            return null;
        }
        ...
        return [
            [
                'phrase' => _p('menu_add_new_quiz'),
                'url' => Phpfox::getLib('url')->makeUrl('quiz.add', [
                    'module' => 'pages',
                    'item' => $aPage['page_id']
                ])
            ]
    ...
    {
        $aQuizs = db()->select('quiz_id, user_id')->from($this->_sTable)->where([
            'module_id' => $sType,
            'item_id' => $iId
        ])->executeRows();
        foreach ($aQuizs as $aQuiz) {
            Phpfox::getService('quiz.process')->deleteQuiz($aQuiz['quiz_id'], $aQuiz['user_id']);
        }
    }
    ...
    {
        if (!Phpfox::getService('pages')->hasPerm($iPage, 'quiz.view_browse_quizzes') || !Phpfox::getUserParam('quiz.can_access_quiz')) {
            return false;
        }
    ...
    {
        return ['quiz.share_quizzes' => _p('who_can_share_quizzes')];
    }
    ...
    {
        if (!Phpfox::getService('groups')->hasPerm($aPage['page_id'], 'quiz.view_browse_quizzes') || !Phpfox::getUserParam('quiz.can_access_quiz')) {
            return null;
        }

        $aMenus[] = [
            'phrase' => _p('quizzes'),
            'url' => Phpfox::getService('groups')->getUrl($aPage['page_id'], $aPage['title'], $aPage['vanity_url']) . 'quiz/',
            'icon' => 'module/core-quiz.png',
            'landing' => 'quiz'
        ];
    ...
    {
        if (!Phpfox::getService('groups')->hasPerm($aPage['page_id'], 'quiz.share_quizzes')
            || !Phpfox::getUserParam('quiz.can_create_quiz')
            || !Phpfox::getService('quiz')->checkLimitation()) {
            return null;
        }
    ...
    public function getNotificationNewItem_Groups($aNotification)
    {
        ...
        $aQuiz = Phpfox::getService('quiz')->getQuizById($aNotification['item_id']);
        if (empty($aQuiz) || empty($aQuiz['item_id']) || ($aQuiz['module_id'] != 'groups')) {
            return false;
        }

        $aRow = Phpfox::getService('groups')->getPage($aQuiz['item_id']);

        if (!isset($aRow['page_id'])) {
            return false;
        }

        $sPhrase = _p('users_add_a_new_quiz_in_the_group_title', [
        ...
        return [
            'link' => Phpfox::getLib('url')->permalink('quiz', $aQuiz['quiz_id'], $aQuiz['title']),
            'message' => $sPhrase,
            'icon' => Phpfox::getLib('template')->getStyle('image', 'activity.png', 'quiz')
        ];
    }
