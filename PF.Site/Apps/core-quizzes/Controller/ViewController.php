    ...
    public function process()
    {
            ...
            } elseif (!isset($aVals['answer']))// check to see all questions have been answered
            {
                ...
                if (is_numeric($iScore)) { // Answers submitted correctly
                    $this->url()->permalink('quiz', $this->request()->get('req2'), $this->request()->get('req3'), true,
                        _p('your_answers_have_been_submitted_and_your_score_is_score', ['score' => $iScore]),
                        ['results', 'id' => Phpfox::getUserId()]);
                } else {
        ...
        if ($this->request()->get('req4') == 'results') {
            ...
            if (Phpfox::getUserParam('quiz.can_post_comment_on_quiz')) {
                $this->template()->assign(['bShowInputComment' => true])
                    ->setHeader([
        ...
        if (isset($aQuiz['module_id']) && !empty($aQuiz['item_id']) && !Phpfox::isModule($aQuiz['module_id'])) {
            return Phpfox_Error::display(_p('Cannot find the parent item.'));
        }

        if (isset($aQuiz['module_id']) && Phpfox::isModule($aQuiz['module_id']) && Phpfox::hasCallback($aQuiz['module_id'], 'checkPermission')) {
            if (!Phpfox::callback($aQuiz['module_id'] . '.checkPermission', $aQuiz['item_id'], 'quiz.view_browse_quizzes')) {
        ...
        if (isset($aQuiz['module_id']) && Phpfox::hasCallback($aQuiz['module_id'], 'getItem')) {
            $aCallback = Phpfox::callback($aQuiz['module_id'] . '.getItem', $aQuiz['item_id']);
            if ($aCallback === false) {
                return Phpfox_Error::display(_p('Cannot find the parent item.'));
            }

            $this->template()
                ->setBreadCrumb(isset($aCallback['module_title']) ? $aCallback['module_title'] : _p($aQuiz['module_id']), $this->url()->makeUrl($aQuiz['module_id']))
                ->setBreadCrumb($aCallback['title'], Phpfox::permalink($aQuiz['module_id'], $aQuiz['item_id']))
                ->setBreadCrumb(_p('quizzes'), $this->url()->makeUrl($aQuiz['module_id'], array($aQuiz['item_id'], 'quiz')));
        } else {
            $this->template()->setBreadCrumb(_p('quizzes'), $this->url()->makeUrl('quiz'));
        }

        $this->template()->setTitle($aQuiz['title'])
            ->setTitle(_p('quizzes'))
            ->setBreadCrumb($aQuiz['title'], $this->url()->permalink('quiz', $aQuiz['quiz_id'], $aQuiz['title']), true)
            ->setMeta('description', _p('full_name_s_quiz_from_time_stamp_title', [
