	...
	{foreach from=$aQuiz.results name=questions item=aQuestion}	
		<div class="quiz_result_item">
			...
			<div class="quiz_result_answer">
				<div class="fw-bold quiz_result_title mb-1">{$aQuestion.questionText|clean}</div>
				<div>
                    {if $aQuestion.userAnswer == $aQuestion.correctAnswer}
					    <div class="correct_answer"><span class="correct_answer_title">{_p var='your_answer_not_dot'}:</span> {$aQuestion.userAnswerText|clean} <span class="txt-success fw-bold status">- {_p var='correct__u'}</span></div>
                    {else}
					    <div class="incorrect_answer"><span class="correct_answer_title">{_p var='your_answer_not_dot'}:</span> {if $aQuestion.userAnswer == 0}<span class="txt-warning">{$aQuestion.userAnswerText|clean}</span>{else}{$aQuestion.userAnswerText|clean}{/if} {if $aQuestion.userAnswer > 0}<span class="txt-danger fw-bold status">- {_p var='incorrect__u'}</span>{/if}</div>
					    <div class="correct_answer"><span class="correct_answer_title">{_p var='correct_answer'}:</span> {$aQuestion.correctAnswerText|clean}</div>
                    {/if}
				</div>
			</div>
		</div>
