...
/**
 * PHP class that extends the PHPfox Storage core.
 * We use
 *
...
class CDN extends \Core\CDN
{
    ...
    public function remove($file)
    {
        ...
        // check and remove on all servers
        foreach ($this->_servers as $server_id => $server) {
            $this->_send([
                'action' => 'remove',
                'file_name' => $file,
                'server_id' => $server_id,
    ...
    private function _send($aPost, $bIsDelete = false)
    {
        ...
        if (!empty($aPost['server_id'])) {
            $server_id = $aPost['server_id'];
        }
        curl_setopt($hCurl, CURLOPT_URL, rtrim($this->_servers[$server_id]['upload'], '/') . '/phpfox-cdn.php');
