...
class GroupMemberResource extends UserResource
{
    ...
    {
        return ['resource_name', 'id', 'full_name', 'avatar', 'is_featured', 'statistic', 'is_owner', 'friendship',
            'friend_id', 'extra', 'is_pending', 'page_id', 'is_admin'];
    }
    ...
    public function getMobileSettings($params = [])
    {
        ...
        if (isset($params['versionName']) && $params['versionName'] != 'mobile' && version_compare($params['versionName'], 'v1.7.1', '>')) {
            $actionMenu = [
                ...
                ['value' => 'group/delete-member', 'style' => 'danger', 'label' => $l->translate('delete_member'), 'acl' => 'groups.can_delete_member'],
                ['value' => 'group/delete-admin', 'style' => 'danger', 'label' => $l->translate('delete_admin'), 'show' => 'is_admin', 'acl' => 'groups.can_delete_admin'],
            ];
            $membershipMenu = [];
        } else {
            $actionMenu = [
    ...
    public function getIsAdmin()
    {
        if (isset($this->rawData['is_admin'])) {
            ...
            $this->is_admin = Phpfox::getService('groups')->isAdmin($this->page_id, $this->getId());
        }
