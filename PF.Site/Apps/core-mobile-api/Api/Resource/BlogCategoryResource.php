...
class BlogCategoryResource extends AbstractCategoryResource
{
    const RESOURCE_NAME = "blog-category";
    ...
    {
        return $this->getShortFields();
    }

    public function getShortFields()
    {
        return [
            'id', 'name', 'subs', 'resource_name'
        ];
    }
    ...
    public function getSubs()
    {
        if (!empty($this->rawData['sub'])) {
            ...
            foreach ($this->rawData['sub'] as $sub) {
                $subs[] = BlogCategoryResource::populate($sub)->toArray();
            }
            return $subs;
        }
        return null;
    ...
    public function getMobileSettings($params = [])
    {
        ...
        $permission = NameResource::instance()->getApiServiceByResourceName(BlogResource::RESOURCE_NAME)->getAccessControl()->getPermissions();
