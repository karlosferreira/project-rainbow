...
class PhpfoxParse implements ParseInterface
{
    ...
    {
        $text = htmlspecialchars_decode(htmlspecialchars($text, ENT_SUBSTITUTE, 'UTF-8'));
        $text = str_replace('�', '', $text);
        if (version_compare('4.7.10', Phpfox::VERSION, '<')) {
            $text = $this->outputLib->cleanPhrases($text);
        }
    ...
    {
        return $this->outputLib->feedStrip($text, $newline);
    }
    ...
    {
        if (is_array($pageId) && isset($pageId[1])) {
            $pageId = $pageId[1];
        }
        $out = '';
        ...
        $service = Phpfox::getService('pages');
        $page = $service->getPage($pageId);
        $url = $service->getUrl($pageId, $page['title'], $page['vanity_url']);
        $userId = $service->getUserId($pageId);
        if (isset($page['title'])) {
            $out = '<span class="user_profile_link_span" id="js_user_name_link_' . $userId . '"><a class="status_user_tag" href="' . $url . '">' . $page['title'] . '</a></span>';
        }
        return $out;
    }
    ...
    {
        if (is_array($groupId) && isset($groupId[1])) {
            $groupId = $groupId[1];
        }
        ...
        $service = Phpfox::getService('groups');
        $group = $service->getPage($groupId);
        $url = $service->getUrl($groupId, $group['title'], $group['vanity_url']);
        $userId = $service->getUserId($groupId);
        if (isset($group['title'])) {
            if ($group['reg_method'] != 2
                || Phpfox::isAdmin()
                || !empty($group['is_liked'])
                || Phpfox::getService('groups')->getPageOwnerId($groupId) == Phpfox::getUserId()
                || Phpfox::getService('groups')->checkCurrentUserInvited($groupId)) {
                $out = '<span class="user_profile_link_span" id="js_user_name_link_' . $userId . '"><a class="status_user_tag" href="' . $url . '">' . $group['title'] . '</a></span>';
            } else {
                $out = $group['title'];
            }
        }
