...
class PanelController extends Phpfox_Component
{
    public function process()
    {
        ...
        $this->search()->setCondition('AND m.viewer_user_id = ' . Phpfox::getUserId() . ' AND m.is_archive = 0 AND m.is_spam = 0');
        list(, $aMessages,) = Phpfox::getService('mail')->get($this->search()->getConditions(), $this->search()->getSort(), $this->search()->getPage(), $iPageSize);

        $sScript = '';
        $iNumberMessage = Phpfox::isUser() ? Phpfox::getService('mail')->getUnseenTotal() : -1;
