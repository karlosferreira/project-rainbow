...
Phpfox::getLib('module')
    ->addAliasNames('pfacebook', 'PHPfox_Facebook')
    ->addServiceNames([
        'phpfoxfb.callback' => Apps\PHPfox_Facebook\Service\Callback::class,
    ]);
/**
...
(new Core\Route('/fb/login'))->run(function (\Core\Controller $Controller) {
    $fb = new Facebook\Facebook([
        'app_id' => setting('m9_facebook_app_id'),
        'app_secret' => setting('m9_facebook_app_secret'),
        'default_graph_version' => 'v8.0'
    ]);
    $helper = $fb->getRedirectLoginHelper();
    $loginUrl = $helper->getLoginUrl($Controller->url->make('/fb/auth'), ['public_profile', 'email']);
...
(new Core\Route('/fb/auth'))->run(function (\Core\Controller $Controller) {
    ...
    if (!empty($accessToken)) {
        ...
        $response = $fb->get('/me?fields=email,name,id,link,first_name,last_name,middle_name', $accessToken);
        $user = $response->getGraphNode();
        if ($user instanceof Facebook\GraphNodes\GraphNode) {
