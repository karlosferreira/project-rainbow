        ...
        if ($aVals = $this->request()->getArray('val')) {
            if ($aVals = $this->_validate($aVals)) {
                ...
                if ($aVals['parent_id'] > 0) {
                    $aRedirectParam = ['parent' => $aVals['parent_id'], 'id' => 'Core_Events'];
                } else {
                    $aRedirectParam = ['id' => 'Core_Events'];
                }
                if ($bIsEdit) {
                    if (Phpfox::getService('event.category.process')->update($aVals)) {
                        $this->url()->send('admincp.app', $aRedirectParam, _p('category_successfully_updated'));
                    }
                } else {
                    if (Phpfox::getService('event.category.process')->add($aVals)) {
                        $this->url()->send('admincp.app', $aRedirectParam, _p('category_successfully_added'));
                    }
                }
            }
        }
