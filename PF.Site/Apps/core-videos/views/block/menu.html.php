...
{if $aItem.canDelete}
    ...
    <li class="item_delete">
        {if !empty($iProfileId)}
            <a href="{url link='video' delete=$aItem.video_id view=$sView user_id=$iProfileId}" class="no_ajax_link sJsConfirm" data-message="{_p var='are_you_sure_you_want_to_delete_this_video_permanently' phpfox_squote=true}" title="{_p('delete_video')}">
                <span class="ico ico-trash-o mr-1"></span>{_p('delete')}
            </a>
        {else}
            <a href="{url link='video' delete=$aItem.video_id view=$sView}" class="no_ajax_link sJsConfirm" data-message="{_p var='are_you_sure_you_want_to_delete_this_video_permanently' phpfox_squote=true}" title="{_p('delete_video')}">
                <span class="ico ico-trash-o mr-1"></span>{_p('delete')}
            </a>
        {/if}
    </li>
