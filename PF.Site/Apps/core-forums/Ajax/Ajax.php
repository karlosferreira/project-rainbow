...
class Ajax extends Phpfox_Ajax
{
    public function addReply()
    {
        ...
        $aVals = $this->get('val');
        $bIsQuickReply = isset($aVals['reply_text']);
        ...
        if (Phpfox::isAppActive('Core_Captcha') && Phpfox::getUserParam('forum.enable_captcha_on_posting')) {
            ...
            if(!Phpfox::getService('captcha')->checkHash($imageVerification)) {
                ...
                $this->call('$(\'#js_forum_reply_submit_btn, #js_forum_form_submit_btn\').removeClass(\'disabled\');');
                $this->call("$('#js_forum_form_submit_btn').attr('disabled', false).removeClass('disabled'); $('#js_quick_reply_form #text').attr('disabled', false).removeClass('disabled'); $('#js_reply_process').html('');");
    ...
    public function deleteThread()
    {
        ...
        if ($bHasAccess) {
            ...
            if ((int)$aThread['group_id'] > 0 && (Phpfox::isAppActive('Core_Pages') || Phpfox::isAppActive('PHPfox_Groups')) && ($sParentId = Phpfox::getPagesType($aThread['group_id'])) && Phpfox::isModule($sParentId)) {
    ...
    public function stickThread()
    {
        ...
        if ((int)$aThread['group_id'] > 0) {
            if (Phpfox::getUserParam('forum.can_stick_thread')) {
                ...
                if ($aThread['group_id'] > 0 && (Phpfox::isAppActive('Core_Pages') || Phpfox::isAppActive('PHPfox_Groups'))) {
    ...
    public function processMerge()
    {
        ...
        $bHasAccess = false;
        $mReturn = false;
        if ((int)$aThread['group_id'] > 0 && (Phpfox::isAppActive('Core_Pages') || Phpfox::isAppActive('PHPfox_Groups'))) {
            $sType = Phpfox::getPagesType($aThread['group_id']);
