...
<form class="form forum-app" method="post" action="{$sFormLink}" id="js_forum_form" onsubmit=" {if PHPFOX_IS_AJAX}  if ({$sGetJsForm}) {l} $Core.processForm('#js_forum_submit_button'); {plugin call='forum.template_controller_post_ajax_onsubmit'}{if isset($iEditId)} $(this).ajaxCall('forum.updateText'); {else} $(this).ajaxCall('forum.addReply'); {/if} {r} return false;{else}{$sGetJsForm}{/if}">
	...
	<div class="form_extra" style="display: block;">
                ...
                {if ($bIsEdit && $aForms.is_announcement != 1) || (!$bIsEdit)}
                    <div class="form-group">
                        ...
                        <div class="label_hover">
                            <select class="form-control close_warning" id="type_id" name="val[type_id]" {if Phpfox::getUserParam('forum.can_post_announcement') || Phpfox::getService('forum.moderate')->hasAccess('' . $iActualForumId . '', 'post_announcement')} onchange="selectThreadType(this);"{/if}>
            ...
            <div class="form-group item-privacy-subscribe">
                <div class="privacy-block-content">
                    <div class="item_is_active_holder">
                        <span class="js_item_active item_is_active"><input type="radio" name="val[is_subscribed]" value="1" class="close_warning v_middle"{value type='radio' id='is_subscribed' default='1' selected='true'}/>{_p var="yes"}</span>
                        <span class="js_item_active item_is_not_active"><input type="radio" name="val[is_subscribed]" value="0" class="close_warning v_middle"{value type='radio' id='is_subscribed' default='0'}/>{_p var="no"}</span>
                    </div>
            ...
            {if Phpfox::getUserParam('forum.can_close_a_thread') || Phpfox::getService('forum.moderate')->hasAccess('' . $iActualForumId . '', 'close_thread')}
                ...
                <div class="form-group" id="js_forum_close">
                    <div class="privacy-block-content">
                        ...
                        <div class="item_is_active_holder">
                            <span class="js_item_active item_is_active"><label><input type="radio" name="val[is_closed]" value="1" class="close_warning v_middle"{value type='radio' id='is_closed' default='1'}/>{_p var="yes"}</label></span>
                            <span class="js_item_active item_is_not_active"><label><input type="radio" name="val[is_closed]" value="0" class="close_warning v_middle"{value type='radio' id='is_closed' default='0' selected='true'}/>{_p var="no"}</label></span>
                        </div>
        ...
        {if isset($iForumId) && $iForumId > 0 && Phpfox::isAppActive('Core_Polls') && Phpfox::getUserParam('poll.can_create_poll') && Phpfox::getUserParam('forum.can_add_poll_to_forum_thread')}
        ...
        {if Phpfox::isModule('tag') && Phpfox::getParam('tag.enable_tag_support') && isset($iForumId) && (!isset($aCallback) || $aCallback === false) }
            {module name='tag.add' sType='forum' bCloseWarning=true}
        {/if}
        {if Phpfox::isAppActive('Core_Captcha') && Phpfox::getUserParam('forum.enable_captcha_on_posting')}
