...
class Process extends Core_Service_Systems_Category_Process
{
    ...
    public function update($aVals, $sName = 'name')
    {
        ...
        if (isset($aVals['name']) && Phpfox::isPhrase($aVals['name'])) {
            ...
            $finalPhrase = $this->addPhrase($aVals);
        }
        ...
        $this->clearGlobalCache($aVals['edit_id']);
    ...
    /**
     * Clear cache for forum
     * @param null $forumId
     */
    public function clearGlobalCache($forumId = null)
    {
        $cacheObject = $this->cache();
        if (!empty($forumId)) {
            $cacheObject->remove('forum_forum_edit_' . $forumId);
        }
        $cacheObject->remove('forum');
    }
