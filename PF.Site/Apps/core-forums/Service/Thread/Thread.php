...
class Thread extends Phpfox_Service
{
    ...
    {
        $bothBlockedUserIds = Phpfox::getService('forum')->getBlockedUserIds();
        ...
        $userJoinParam = 'u.user_id = ft.user_id' . (!empty($bothBlockedUserIds) ? ' AND u.user_id NOT IN (' . implode(',', $bothBlockedUserIds) . ')' : '');
        if (isset($bLeftJoinQuery)) {
            $this->database()->leftJoin(Phpfox::getT('user'), 'u', $userJoinParam);
        } else {
            $this->database()->join(Phpfox::getT('user'), 'u', $userJoinParam);
        }
        ...
        foreach ($aThreads as $iKey => $aThread) {
            ...
            if ($aThread['is_closed']) {
                ...
                if (!$aThread['is_seen']) {
                    ...
                    else if ((PHPFOX_TIME - Phpfox::getParam('forum.keep_active_posts') * 60) > $aThread['time_update']) {
                        $sCss = 'old';
                    } else if (!empty($aThread['time_update']) && Phpfox::isUser() && $aThread['time_update'] < Phpfox::getCookie('last_login')) {
                        $sCss = 'old';
                    }
                } else {
                    // New post was added
    ...
    {
        if (Phpfox::isAppActive('Core_Pages') || Phpfox::isAppActive('PHPfox_Groups')) {
        ...
        if (!isset($aRow['forum_is_closed']) || !$aRow['forum_is_closed']) {
            ...
            if (Phpfox::isAppActive('Core_BetterAds')) {
                $aRow['canSponsor'] = !$aRow['view_id'] && !$aRow['is_announcement'] && Phpfox::getUserParam('forum.can_sponsor_thread');
                $bCanPurchaseSponsor = $this->canPurchaseSponsorItem($aRow['thread_id']);
                $sCurrentPrice = Phpfox::getService('forum')->getSponsorPrice();
                $aRow['canPurchaseSponsor'] = !$aRow['view_id'] && !$aRow['is_announcement'] && (Phpfox::getUserParam('forum.can_purchase_sponsor') && (isset($sCurrentPrice) && $sCurrentPrice != '')) && (Phpfox::getUserId() == (int)$aRow['user_id']) && $bCanPurchaseSponsor;
            }
            ...
            $aRow['hasPermission'] = $aRow['canEdit'] || $aRow['canDelete'] || $aRow['canStick']
                || $aRow['canClose'] || $aRow['canMerge'] || $aRow['canMove'] || $aRow['canReply']
                || $aRow['canCopy'] || $aRow['canApprove'] || $aRow['canSponsor'] || $aRow['canPurchaseSponsor'];
        } else {
            if ($aCallback === null) {
            ...
            $aRow['hasPermission'] = $aRow['canDelete'];
        }
    }
    ...
    public function getAnnoucements($iForumId = null, $iGroupId = null, $iLimit = 10)
    {
        if ($iForumId !== null) {
            $aAnnouncements = $this->database()->select('ft.*, fs.subscribe_id AS is_subscribed, fa.announcement_id, ' . Phpfox::getUserField())
                ...
                ->where('fa.forum_id = ' . (int)$iForumId.' AND ft.view_id = 0')
                ->limit($iLimit)
    ...
    public function getForEdit($iId)
    {
        ...
        } else if ((int)$aThread['order_id'] == 1) {
            $aThread['type_id'] = 'sticky';
        } else if ((int)$aThread['order_id'] == 2) {
            $aThread['type_id'] = 'sponsor';
        }

        $aThread['total_attachment'] = Phpfox::isModule('attachment') ? Phpfox::getService('attachment')->getCountForItem($aThread['start_id'], 'forum') : 0;
    ...
    )
    {
        ...
        return [count($aThreads), $aThreads];
    ...
    public function getRandomSponsored($iLimit = 4, $iCacheTime = 5)
    {
        ...
        $aThreads = $this->database()->select('ft.*, ft.total_view AS forum_thread_total_view, fpt.text, fpt.text_parsed, ' . Phpfox::getUserField() . ', s.*')
            ->from($this->_sTable, 'ft')
            ->join(Phpfox::getT('user'), 'u', 'u.user_id = ft.user_id')
            ->join(Phpfox::getT('forum_post'), 'fp', 'ft.thread_id = fp.thread_id AND fp.title != \'\'')
            ->join(Phpfox::getT('forum_post_text'), 'fpt', 'fpt.post_id = fp.post_id')
        ...
        foreach ($aThreads as $key => $aThread) {
            $aThreads[$key]['total_view'] = $aThread['forum_thread_total_view'];
        }
    ...
    {
        if (is_string($aThreadCondition)) {
            $aThreadCondition = [$aThreadCondition];
        }

        if (!empty($blockedUserIds = Phpfox::getService('forum')->getBlockedUserIds())) {
            $aThreadCondition[] = 'AND (ft.user_id NOT IN (' . implode(',', $blockedUserIds) . '))';
        }
        ...
        $mConditions[] = (!empty($blockedUserIds) ? 'fp.user_id NOT IN (' . implode(',', $blockedUserIds) . ') AND ' : '') . 'fp.thread_id = ' . $aThread['thread_id'] . $sViewId;
        ...
        foreach ($aThread['posts'] as $iKey => $aPost) {
            ...
            $aThread['posts'][$iKey]['last_update_on'] = _p('last_update_on_time_stamp_by_update_user', [
            ...
            if (!empty($aThread['post_starter']) && $aThread['post_starter']['post_id'] == $aPost['post_id']) {
                $iFirstPostKey = $iKey;
            }
        ...
        if (!empty($sPostIds)) {
            list(, $aAttachments) = Phpfox::isModule('attachment') ? Phpfox::getService('attachment')->get('attachment.item_id IN(' . $sPostIds . ') AND attachment.view_id = 0 AND attachment.category_id = \'forum\' AND attachment.is_inline = 0',
        ...
        if (!empty($aThread['post_starter']) && isset($iFirstPostKey)) {
