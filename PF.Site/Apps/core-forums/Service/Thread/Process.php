...
class Process extends Phpfox_Service
{
    ...
    public function update($iId, $iUserId, $aVals)
    {
        ...
        //Support api to remove Poll
        if (!empty($aVals['remove_poll'])) {
            db()->update(Phpfox::getT('forum_thread'), ['poll_id' => '0'], ['thread_id' => $iId]);
        }

        if (!empty($aVals['poll_id']) && Phpfox::isAppActive('Core_Polls') && Phpfox::getUserParam('poll.can_create_poll')) {
            $aUpdate['poll_id'] = (int)$aVals['poll_id'];
            $this->database()->update(Phpfox::getT('poll'), ['item_id' => $iId],
        ...
        $this->database()->update(Phpfox::getT('forum_post'), [
        ...
        if (Phpfox::isModule('feed')) {
            ...
            foreach ($aPosts as $aPost) {
                $feedProcess->update('forum_post', $aPost['post_id'], serialize([
    ...
    public function move($iThread, $iForum)
    {
        ...
        foreach (Phpfox::getService('forum')->id($aThread['forum_id'])->getParents() as $iForumId) {
            if (isset($aOldThread['thread_id'])) {
                ...
                $this->database()->update(Phpfox::getT('forum'),
                    ['thread_id' => 0, 'post_id' => 0, 'last_user_id' => 0], 'forum_id = ' . $iForumId);
            }
        }
    ...
    public function copy($iThread, $iForum, $sTitle = null)
    {
        ...
        $subscriberIds = db()->select('user_id')
        ...
        if (!empty($subscriberIds)) {
            $multiInsert = [];
            foreach (array_column($subscriberIds, 'user_id') as $userId) {
                $multiInsert[] = [
            ...
            db()->multiInsert(Phpfox::getT('forum_subscribe'), ['thread_id', 'user_id'], $multiInsert);
        }
    ...
    {
        static $iLoop = 0;
        $ignoreCallback = $aCallback === false;
        ...
        if ($this->_bPassed === false) {
            ...
            if ($ignoreCallback) {
                if (isset($aVals['type_id']) && $aVals['type_id'] == 'announcement' && (Phpfox::getUserParam('forum.can_post_announcement') || Phpfox::getService('forum.moderate')->hasAccess($aVals['forum_id'],
                    ...
                    $iId = 0;
                    foreach (array_merge([$aVals['announcement_forum_id']],
                        (is_array($aChildren) ? $aChildren : [])) as $iForumid) {
                        $aVals['forum_id'] = $iForumid;

                        if ($iId = $this->add($aVals)) {
                            $this->database()->insert(Phpfox::getT('forum_announcement'),
                                ['forum_id' => $iForumid, 'thread_id' => $iId]);
                        }
                    }
        ...
        $aInsert = [
            'forum_id' => ($ignoreCallback ? $aVals['forum_id'] : 0),
            'group_id' => ($ignoreCallback ? 0 : (int)$aCallback['item']),
        ...
        $this->database()->update($this->_sTable, ['start_id' => $iPostId], 'thread_id = ' . $iId);

        if ($ignoreCallback && !isset($bSkipFeedEntry)) {
        ...
        if (!isset($bSkipFeedEntry) && empty($aExtra) && (defined('PHPFOX_UNIT_TEST') || $iLoop === 1)) {
            ((Phpfox::isModule('feed') && !defined('PHPFOX_SKIP_FEED_ENTRY') && Phpfox::getParam('forum.forum_allow_create_feed_when_add_new_item', 1)) ? Phpfox::getService('feed.process')->callback($aCallback)->add('forum',
                $iId, 0, 0, ($ignoreCallback ? 0 : $aCallback['item'])) : null);
    ...
    {
        $aThread = $this->database()->select('ft.forum_id, ft.group_id, ft.thread_id, ft.user_id, ft.poll_id, ft.view_id, ft.order_id')
            ->from($this->_sTable, 'ft')
            ->where('ft.thread_id = ' . (int)$iThread)
            ->execute('getSlaveRow');
        ...
        if (isset($aLastThread['thread_id'])) {
            ...
            if ($aThread['forum_id'] > 0) {
                $this->database()->update(Phpfox::getT('forum'),
                    ['thread_id' => 0, 'post_id' => 0, 'last_user_id' => 0],
        ...
        if ($aThread['poll_id'] > 0 && Phpfox::isAppActive('Core_Polls')) {
        ...
        if ((isset($aThread['is_sponsor']) && $aThread['is_sponsor'] == 1) || $aThread['order_id'] == 2) {
            $this->cache()->remove('forum_thread_sponsored');
        }
    ...
    {
        $this->database()->update($this->_sTable, ['order_id' => $iType], 'thread_id = ' . (int)$iThread);
        return true;
    }

    public function close($iThread, $iType = 1)
    {
        $this->database()->update($this->_sTable, ['is_closed' => $iType], 'thread_id = ' . (int)$iThread);
    ...
    public function merge($iThread, $iNewForumId, $sUrl)
    {
        ...
        $this->database()->update(Phpfox::getT('forum_post'), ['thread_id' => $aOldThread['thread_id']],
        ...
        if (!$bIsGroup) {
            ...
            foreach (Phpfox::getService('forum')->id($iNewForumId)->getParents() as $iForumId) {
                ...
                $this->database()->update(Phpfox::getT('forum'),
                    ['thread_id' => $aOldThread['thread_id'], 'post_id' => 0], 'forum_id = ' . $iForumId);
            }
        ...
        return Phpfox_Url::instance()->makeUrl('forum',
            [$aForum['name_url'] . '-' . $aForum['forum_id'], $aOldThread['title_url']]);
    }
    ...
    public function markRead($iForumId, $iGroupId = 0)
    {
        ...
        list(, $aThreads) = Phpfox::getService('forum.thread')->get(['ft.forum_id = ' . (int)$iForumId . ' AND ft.group_id = ' . (int)$iGroupId . ' AND ft.view_id = 0']);
    ...
    public function updateTrack($iThread, $bNoCache = false)
    {
        ...
        if (!$cache->exists() || $bNoCache) {
            $this->database()->update(Phpfox::getT('forum_thread'), ['total_view' => ['= total_view +', 1]],
                'thread_id = ' . (int)$iThread);
            $cache->set('set');
        }
    }
    ...
    public function sponsor($iThreadId, $iType)
    {
        ...
        $iType = (int)$iType;
        $this->database()->update($this->_sTable,
            ['order_id' => ($iType == 2 ? 2 : 0)], // 2 : sponsored; 1 : featured; 0 : normal
    ...
    public function approve($iThreadId)
    {
        ...
        $this->database()->update(Phpfox::getT('forum_thread'), ['view_id' => '0'],
            'thread_id = ' . (int)$iThreadId);
        if (!$aThread['group_id']) {
            foreach (Phpfox::getService('forum')->id($aThread['forum_id'])->getParents() as $iForumid) {
                $this->database()->update(Phpfox::getT('forum'),
                    ['thread_id' => $iThreadId, 'post_id' => 0, 'last_user_id' => $aThread['user_id']],
                    'forum_id = ' . $iForumid);
