...
class Callback extends Phpfox_Service
{
    ...
    public function getNotificationSettings()
    {
        return [
            ...
            'forum.email_notification' => [
                'phrase' => _p('other_notifications_of_forums'),
                'default' => 1
            ],
        ];
    }
    ...
    public function addLikePost($iItemId, $bDoNotSendEmail = false)
    {
        ...
        if (!$bDoNotSendEmail) {
            ...
            Phpfox::getLib('mail')->to($aRow['user_id'])
                ...
                ->message(array(
                    'content_full_name_liked_one_of_your_forum_posts_in',
    ...
    public function getNotificationThread_Approved($aNotification)
    {
        ...
        return [
            'link' => Phpfox_Url::instance()
                ->permalink('forum.thread', $aRow['thread_id'], $aRow['title'], false),
            'message' => $sPhrase,
            'icon' => Phpfox_Template::instance()->getStyle('image', 'activity.png', 'blog')
        ];
    }
    ...
    public function getNotificationPost_Approved($aNotification)
    {
        ...
        return [
            'link' => Phpfox_Url::instance()
                ->permalink('forum.thread', $aRow['thread_id'], $aRow['title'], false).'view_'.$aRow['post_id'],
            'message' => $sPhrase,
            'icon' => Phpfox_Template::instance()->getStyle('image', 'activity.png', 'blog')
        ];
    }
    ...
    public function getActivityFeedReply($aItem, $aCallback = null, $bIsChildItem = false)
    {
        ...
        $bHasPerm = false;
        if ((Phpfox::isAppActive('Core_Pages') || Phpfox::isAppActive('PHPfox_Groups')) && $sPageType = Phpfox::getPagesType($aRow['group_id'])) {
        ...
        $sTitle = Phpfox::getLib('parse.output')->clean($aRow['title']);
        ...
        $parseOutputObject = Phpfox::getLib('parse.output');

        $aReturn = array(
            'feed_title' => '',
            'feed_info' => $sPhrase,
            'feed_link' => $sLink,
            'feed_status' => $aRow['text'],
    ...
    public function getActivityFeed($aItem, $aCallBack = null, $bIsChildItem = false)
    {
        ...
        $sGroupType = ($aRow['group_id'] > 0 && (Phpfox::isAppActive('Core_Pages') || Phpfox::isAppActive('PHPfox_Groups'))) ? Phpfox::getPagesType($aRow['group_id']) : '';
        if (((defined('PHPFOX_IS_PAGES_VIEW') && defined('PHPFOX_PAGES_ITEM_TYPE') && Phpfox::getService(PHPFOX_PAGES_ITEM_TYPE)->hasPerm(null,
                        'forum.view_browse_forum') == false)
                || (!defined('PHPFOX_IS_PAGES_VIEW') && $aRow['group_id'] > 0 && $sGroupType == 'pages' && Phpfox::isAppActive('Core_Pages') && !Phpfox::getService('pages')->hasPerm($aRow['group_id'],
                        'forum.view_browse_forum')))
        ...
        $sLink = Phpfox::permalink('forum.thread', $aRow['thread_id'], $aRow['title'], false);
        ...
        $aRow['breadcrumb'] = $aForum ? $aForum['breadcrumb'] : '';
    ...
    public function getPagePerms()
    {
        ...
        $aPerms['forum.reply_forum'] = _p('who_can_reply_a_discussion');
    ...
    public function getGroupPerms()
    {
        $aPerms = [
            ...
            'forum.reply_forum' => _p('who_can_reply_a_discussion')
        ];
        return $aPerms;
    }
