...
class Forum extends Phpfox_Service
{
     ...
     * @return string
     */
    public function getModifiedJumpTool($iForumId = 0, $bIdOnly = false, $bIsEditing = false, $aValues = array(), $bSelectAll = false)
    {
        $sOptions = '';
        $aForum = db()->select('forum_id, name')
            ->from(':forum')
            ->where([
                'forum_id' => $iForumId
            ])
            ->executeRow();
        $sOptions .= '<option value="' . ($bIdOnly ? $aForum['forum_id'] : Phpfox_Url::instance()->permalink('forum', $aForum['forum_id'],
                _p($aForum['name']))) . '"' . (($bSelectAll || $this->_iActive == $aForum['forum_id'] || in_array($aForum['forum_id'],
                    $aValues)) ? ' selected="selected"' : '') . '>' . Phpfox_Locale::instance()->convert(_p($aForum['name'])) . '</option>' . "\n";
        $sOptions .= $this->_getFromCache()->_buildJump($iForumId, $bIdOnly, 0, $bIsEditing, $aValues, $bSelectAll);
        return $sOptions;
    }
    ...
    private function _getFromCache($bNoCache = false)
    {
        ...
        if (!($this->_aForums = $this->cache()->get($sCacheId)) || $bNoCache) {
            $aForums = $this->database()->select('f.forum_id, f.parent_id, f.view_id, f.is_category, f.name, f.name_url, f.description, f.is_closed, f.thread_id, f.total_thread, f.total_post, f.post_id')
                ->from($this->_sTable, 'f')
                ->where('f.view_id = 0')
     ...
     * @return array
     */
    public function getForums($parentId = 0)
    {
        $aForums = $this->_getFromCache()->_buildForum($parentId);
     ...
     * @return bool|mixed
     */
    public function getForum($noCache = false)
    {
        $this->_getFromCache($noCache);
    ...
    /**
     * Get both blocked user ids
     * @param null $userId
     * @return array
     */
    public function getBlockedUserIds($userId = null)
    {
        static $blockedUserIds = [];
        empty($userId) && $userId = Phpfox::getUserId();

        if (isset($blockedUserIds[$userId])) {
            return $blockedUserIds[$userId];
        }

        $blockedUserIds[$userId] = Phpfox::getService('user.block')->get($userId, true);
