...
class ForumController extends Phpfox_Component
{
    ...
    public function process()
    {
        ...
        if ($this->request()->get('req2') == 'tag' && $this->request()->get('req5') && $this->request()->get('module')) {
            if ($aCallback = Phpfox::getService('groups')->getPage($this->request()->get('item'))) {
                $bIsSearch = true;
                $bIsTagSearch = true;
                $bIsModuleTagSearch = true;
                $aCallback['url_home'] = Phpfox::getService('groups')->getUrl($aCallback['page_id'], $aCallback['title'], $aCallback['vanity_url']) . '/forum/';
            }
        }
        ...
        if ($iResult || isset($bForceResult)) {
            ...
            list($iCnt, $aThreads) = Phpfox::getService('forum.post')->callback($aCallback)->isSearch($isSearch)->isAdvSearch($bIsAdvSearch)
                ...
                ->isNewSearch($sView == 'new')
                ->isSubscribeSearch($sView == 'subscribed')
         ...
         */
        $bIsAdmin = false;
        if (!empty($aParentModule['module_id']) && !empty($aParentModule['item_id']) && Phpfox::hasCallback($aParentModule['module_id'], 'isAdmin')) {
        ...
        if ($bIsSearch && (isset($bForceResult) || $iResult)) {
            if (isset($bForceResult)) {
                if ($bCanDeleteAll) {
                    $aModerationMenu[] = [
                        'phrase' => _p('delete'),
                        'action' => 'delete',
                        'message' => _p('are_you_sure_you_want_to_delete_selected_threads_permanently'),
                    ];
                }
                ...
                if (count($aModerationMenu)) {
                    $this->setParam('global_moderation', [
                            ...
                            'extra' => '&module_id=' . (isset($aCallback['module_id']) ? $aCallback['module_id'] : '') . '&item_id=' . (isset($aCallback['item_id']) ? $aCallback['item_id'] : '')
                        ]
                    );
                    $bShowModerator = true;
                }
            ...
            if (count($aModerationMenu)) {
                $sExtra = '';
                if (!empty($aCallback['module_id']) && !empty($aCallback['item_id'])) {
                    $sExtra = "&module_id=$aCallback[module_id]&item_id=$aCallback[item_id]";
                }
                $this->setParam('global_moderation', [
                    'name'  => 'forum',
