    ...
    public function process()
    {
        ...
        if (($iRedirect = $this->request()->getInt('redirect')) && ($aThread = Phpfox::getService('forum.thread')->getForRedirect($iRedirect))) {
            if ((Phpfox::isModule('pages') || Phpfox::isModule('groups')) && $aThread['group_id'] > 0 && ($sParentId = Phpfox::getPagesType($aThread['group_id'])) && Phpfox::isModule($sParentId)) {
                ...
                if (isset($aCallback['module'])) {
                    ...
                    $this->url()->send($aCallback['url_home'], ['forum', $aThread['title_url']]);
                }
            }
            $this->url()->send('forum',
                [$aThread['forum_url'] . '-' . $aThread['forum_id'], $aThread['title_url']]);
        }
        ...
        if ($aCallback === null) {
            $aForum = Phpfox::getService('forum')
                ...
                ->getForum();
            if ($aForum) {
                $forumId = $aForum['forum_id'];
                $this->setParam('iActiveForumId', $forumId);
            }
        }
        ...
        if ($aForum) {
            ...
            $this->template()
                ...
                ->setBreadCrumb(Phpfox_Locale::instance()->convert(_p($aForum['name'])), $this->url()->permalink('forum', $forumId, $aForum['name']));
        } else {
        ...
        if ($aForum) {
            $sCurrentThreadLink = $this->url()->makeUrl('forum', [
                $aForum['name_url'] . '-' . $forumId,
                $aThread['title_url']
            ]);
        } else {
            $sCurrentThreadLink = $this->url()->makeUrl($aCallback['url_home'], $aThread['title_url']);
        }
        ...
        $this->template()->setTitle($aThread['title'])
            ...
            ->setMeta('description', $aThread['title'] . (!empty($aForum['name']) ? ' - ' . _p($aForum['name']) : ''))
            ...
            ->assign([
                    ...
                    'sShareDescription'  => !empty($aThread['post_starter']) ? str_replace(["\n", "\r", "\r\n"], '', $aThread['post_starter']['text']) : '',
                    'aTitleLabel'        => $aTitleLabel,
                ]
            );
