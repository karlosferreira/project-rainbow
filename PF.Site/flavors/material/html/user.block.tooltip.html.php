...
<div class="user_tooltip_cover" style="background-image:url('{$aUser.cover_photo_link}')"></div>
    ...
    <div class="user_tooltip_info">
        ...
        <div class="user_tooltip_info_up">
            <a href="{$aUser.profile_link}" class="user_tooltip_info_user">
...
{if $aUser.user_id != Phpfox::getUserId() && !$bIsPage}
    ...
    <div class="user_tooltip_action_user">
        {if isset($aUser.is_friend) && $aUser.is_friend === 2}
            <div class="friend_request_sent">
                <span class="ico ico-arrow-right mr-1"></span>{_p var='friend_request_sent'}
            </div>
        {/if}
        <ul>
            {if empty($aUser.is_ignore_request) && !$bLoginAsPage && Phpfox::isUser() && Phpfox::isModule('friend') && Phpfox::getUserParam('friend.can_add_friends') && empty($aUser.is_friend) && Phpfox::getService('user.privacy')->hasAccess('' . $aUser.user_id . '', 'friend.send_request')}
                <li class="py-1"><a class="btn btn-primary btn-sm btn-icon" href="#" onclick="$(this).closest('.js_user_tool_tip_holder').hide();return $Core.addAsFriend('{$aUser.user_id}');" title="{_p var='add_to_friends'}"><span class="ico ico-user1-plus-o"></span>{_p var='add_friends'}</a></li>
            {/if}
            {if $bShowBDay}
                <li class="py-1"><a class="btn btn-default btn-sm btn-icon" href="{url link=$aUser.user_name}"><span class="ico ico-birthday-cake-alt mr-1"></span>{_p var='birthday_wishes'}</a></li>
                {if $bCanSendMessage}
                    <li class="py-1">
                        <div class="item-tooltip-viewmore-button dropup">
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a  href="#" onclick="$Core.composeMessage({l}user_id: {$aUser.user_id}{r});$(this).closest('.js_user_tool_tip_holder').hide(); return false;"><span class="ico ico-comment-o"></span>{_p var='send_message'}</a></li>
                            </ul>
                        </div>
                    </li>
                {/if}
            {elseif $bCanSendMessage}
                <li class="py-1"><a class="btn btn-default btn-sm" href="#" onclick="$Core.composeMessage({l}user_id: {$aUser.user_id}{r});$(this).closest('.js_user_tool_tip_holder').hide(); return false;">{_p var='send_message'}</a></li>
            {/if}
        </ul>
    </div>
{/if}
